...
// ═══ Η ΕΤΙΚΕΤΑ ΕΠΙΖΕΙ ΤΗΣ ΣΗΜΑΝΣΗΣ «ΠΛΗΡΩΘΗΚΕ» ═══════════════════════════
// ΤΟ ΚΕΝΟ ΠΟΥ ΑΦΗΣΕ ΤΟ ΣΦΑΛΜΑ ΝΑ ΖΗΣΕΙ: όλοι οι έλεγχοι από πάνω ρωτούσαν την
// ΟΜΑΔΑ, δηλαδή αν εκπίπτει. Κανείς δεν ρώτησε πώς ΛΕΓΕΤΑΙ η δαπάνη που
// γεννιέται. Ετσι, 21 από τις 28 κατηγορίες έχαναν το όνομά τους και 15 από
// αυτές γίνονταν «Άλλο», με πράσινη σουίτα.
//
// Η φόρμα «Εκκρεμής υποχρέωση» γράφει slug ταξινομίας στον λογαριασμό, οπότε
// ΚΑΘΕ slug πρέπει να γυρίζει πίσω με το όνομά του.
{
  let lost = '';
  for (const slug of Object.keys(BY_SLUG)) {
    const { cat } = billCategory(slug);
    if (cat !== BY_SLUG[slug].label) { lost = `${slug}: «${BY_SLUG[slug].label}» → «${cat}»`; break; }
  }
  eq('κάθε slug ταξινομίας κρατά την ετικέτα του', lost, '');
}
{
  // Και τα παλιά κλειδιά των λογαριασμών δίνουν ετικέτα που η ταξινομία
  // αναγνωρίζει: αλλιώς η δαπάνη γεννιέται με κατηγορία που καμία οθόνη δεν
  // μπορεί να ξαναδιαλέξει.
  let orphan = '';
  for (const key of Object.keys(EXPENSE_MAP)) {
    const { cat } = billCategory(key);
    if (!resolveCategory(cat)) { orphan = `${key} → «${cat}»`; break; }
  }
  eq('καμία ετικέτα εκτός ταξινομίας', orphan, '');
}
eq('ΕΝΦΙΑ από slug κρατά το όνομά του', billCategory('enfia').cat, 'ΕΝΦΙΑ');
eq('ΕΝΦΙΑ από παλιό κλειδί λογαριασμού το ίδιο', billCategory('taxes').cat, 'ΕΝΦΙΑ');
