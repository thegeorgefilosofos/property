// ═══════════════════════════════════════════════════════════════════════════
// ΤΙ ΒΛΕΠΕΙ ΑΥΤΟΣ Ο ΧΡΗΣΤΗΣ, ΤΩΡΑ.
//
// ΤΟ ΠΡΟΒΛΗΜΑ ΠΟΥ ΛΥΝΕΙ
// Η ορατότητα κρινόταν σε ΔΥΟ σημεία που δεν ήξεραν το ένα το άλλο:
//   • isTabRelevant  — κοιτούσε μόνο το προφίλ (ιδιώτης / επαγγελματίας)
//   • tabFitsStatus  — κοιτούσε μόνο την κατάσταση του ακινήτου
// Κανένα από τα δύο δεν ήξερε ΠΟΣΑ ακίνητα έχει ο χρήστης ούτε αν είναι φυσικό
// ή νομικό πρόσωπο. Αποτέλεσμα: ο ιδιοκτήτης ενός σπιτιού που μένει ο ίδιος
// έβλεπε «Σύγκριση ακινήτων» με ένα ακίνητο, «Τιμολόγηση» για ακίνητο που δεν
// νοικιάζει και λογιστικά επιχειρήσεων ενώ δεν έχει επιχείρηση.
//
// ΤΡΕΙΣ ΕΙΣΟΔΟΙ, ΜΙΑ ΑΠΟΦΑΣΗ
//   1. Η ΚΑΤΑΣΤΑΣΗ του ακινήτου — τι κάνεις μ' αυτό το σπίτι
//   2. Ο ΑΡΙΘΜΟΣ των ακινήτων — και αν συγκρίνονται μεταξύ τους
//   3. Η ΝΟΜΙΚΗ ΜΟΡΦΗ — φυσικό ή νομικό πρόσωπο
//
// ΓΙΑΤΙ ΚΡΥΒΟΥΜΕ ΑΝΤΙ ΝΑ ΚΛΕΙΔΩΝΟΥΜΕ
// Μια καρτέλα που δεν έχει νόημα δεν εμφανίζεται καθόλου. Δεν μπαίνει
// γκριζαρισμένη ούτε με λουκέτο: το λουκέτο σημαίνει «πλήρωσε για να το δεις»
// και είναι ψέμα όταν το εργαλείο δεν αφορά καν αυτόν τον χρήστη. Ο ιδιοκτήτης
// που μένει στο σπίτι του δεν πρόκειται ΠΟΤΕ να χρειαστεί τιμολόγηση ανά
// διανυκτέρευση και το να τη βλέπει κλειδωμένη τον κάνει να νιώθει ότι κάτι
// χάνει.
//
// ΚΑΘΕ ΑΠΟΦΑΣΗ ΕΧΕΙ ΛΟΓΟ ΓΡΑΜΜΕΝΟ. Η `explain` επιστρέφει γιατί κάτι δεν
// φαίνεται, με λόγια. Χωρίς αυτό, το πρώτο «γιατί λείπει το Χ;» γίνεται μισή
// μέρα ψάξιμο σε τρία αρχεία.
// ═══════════════════════════════════════════════════════════════════════════

import { readStatus, type PropertyStatus, type StatusRow } from './status';
import { NAV_LABELS } from '../nav/labels';
import { type LegalForm } from '../accounting/dossier';

// ═══════════════════════════════════════════════════════════════════════════
// ΕΝΑΣ ΤΥΠΟΣ ΝΟΜΙΚΗΣ ΜΟΡΦΗΣ ΣΕ ΟΛΗ ΤΗΝ ΕΦΑΡΜΟΓΗ.
// ─────────────────────────────────────────────────────────────────────────
// Εδώ ζούσε δεύτερος τύπος με το ΙΔΙΟ όνομα `LegalForm`, με δύο τιμές αντί για
// τέσσερις. Η σύγκρουση φαινόταν γυμνή στο app/dashboard/page.tsx, που έπρεπε
// να εισάγει και τους δύο και να μετονομάσει τον έναν:
//
//     import { type LegalForm } from '@/lib/property/visibility';
//     import { type LegalForm as DossierLegalForm } from '@/lib/accounting/dossier';
//
// Και κόστιζε ΔΥΟ states γεμισμένα απο την ΙΔΙΑ τιμή της βάσης — δύο εντολές
// που μπορούσαν να ξεσυγχρονιστούν, για μια πληροφορία που είναι μία.
//
// Η δυαδική περίληψη δεν ήταν λάθος ιδέα· ήταν λάθος ΟΝΟΜΑ. Η ορατότητα δεν
// ρωτά «είναι εταιρεία;» — ρωτά «ασκεί επιχειρηματική δραστηριότητα;» και η
// απάντηση για ατομική επιχείρηση είναι ναι. Οποιος τη ρωτάει, τη ρωτάει στο
// `HAS_BUSINESS` του dossier, εκεί όπου ορίζονται οι μορφές. Εδώ δεν τη ρωτάει
// πια κανείς: την είχε ανάγκη μόνο το `accountingSections`, που έφυγε.
// ═══════════════════════════════════════════════════════════════════════════
export type { LegalForm } from '../accounting/dossier';

export interface PropertyLike extends StatusRow {
  id: string;
  prop_type?: string | null;
  sqm?: number | null;
  year_built?: number | null;
  postal_code?: string | null;
}

export interface OwnerContext {
  legalForm: LegalForm;
  properties: readonly PropertyLike[];
}

// ── ΟΙ ΚΑΡΤΕΛΕΣ ────────────────────────────────────────────────────────────

// ΔΥΟ ΚΑΤΑΛΟΓΟΙ ΠΟΥ ΔΙΑΒΑΖΟΝΤΑΝ ΩΣ ΑΥΘΕΝΤΙΑ ΚΑΙ ΔΕΝ ΤΟΥΣ ΔΙΑΒΑΖΕ ΚΑΝΕΙΣ.
// Εδώ κάθονταν το `ALWAYS` («πάντα ορατές») και το `GATED_TABS` («ο πλήρης
// κατάλογος όσων μπορούν να κρυφτούν»). Καμία γραμμή της εφαρμογής δεν τα
// ρωτούσε: η απάντηση βγαίνει από το `decide`, παρακάτω. Όποιος πρόσθετε
// καρτέλα στο `ALWAYS` θα περίμενε να εμφανιστεί και δεν θα εμφανιζόταν ποτέ.
// Ο κανόνας ζει σε ΕΝΑ σημείο: κάθε ΥΠΑΡΚΤΗ καρτέλα που δεν αναφέρεται σε
// συνθήκη φαίνεται. Το «υπαρκτή» δεν ήταν γραμμένο πουθενά και αυτό ήταν το
// σφάλμα που κλείνει ο κατάλογος αμέσως από κάτω.

// ── ΠΟΙΟΙ ΚΩΔΙΚΟΙ ΥΠΑΡΧΟΥΝ ΚΑΝ ──────────────────────────────────────────────
//
// Η `tabDecision` τελείωνε με `return { visible: true }` για ΟΤΙΔΗΠΟΤΕ δεν
// έπεφτε σε κανόνα. Δηλαδή η προεπιλογή για έναν κωδικό που δεν υπάρχει ήταν
// «ναι, δείξ' τον».
//
// ΤΙ ΕΣΠΑΣΕ ΣΤΗΝ ΟΘΟΝΗ, ΟΧΙ ΘΕΩΡΗΤΙΚΑ:
//
//   · Η καρτέλα διαβάζεται από το hash της διεύθυνσης (lib/nav/history.ts). Η
//     `parseNavHash` δέχεται ό,τι ταιριάζει στο /^[a-zA-Z0-9_-]+$/ — δεν ξέρει
//     τι είναι καρτέλα και σωστά: δουλειά της είναι η μορφή. Άρα το «#finance»
//     (ένα γράμμα λείπει από το «#finances») έφτανε ως κωδικός καρτέλας.
//   · Στο app/dashboard/page.tsx το `navSafe` γυρίζει στην Επισκόπηση ΜΟΝΟ όταν
//     το `navVisible` πει όχι. Με «ναι» για τα πάντα δεν γύριζε ποτέ: ο χρήστης
//     έμενε σε καρτέλα «finance», με κεφαλίδα και κουμπί «πίσω» και σώμα
//     ΚΕΝΟ — κάθε συνθήκη απόδοσης είναι σύγκριση ακριβούς κωδικού και καμία
//     δεν ταίριαζε. Καμία εξήγηση, κανένα σφάλμα στην κονσόλα.
//   · Και ένας ΖΩΝΤΑΝΟΣ κωδικός-φάντασμα: ο βοηθός στέλνει ακόμη
//     `tab: 'expenses'` (PropertyAssistant.tsx, δύο σημεία). Οι δαπάνες
//     συγχωνεύτηκαν στο `finances` και το `expenses` έπαψε να είναι καρτέλα —
//     το λέει ρητά το σχόλιο του NAV_ICON στη σελίδα. Το κουμπί «άνοιγμα»
//     περνούσε τον έλεγχο και οδηγούσε στην ίδια κενή οθόνη.
//
// ΓΙΑΤΙ ΤΟ «ΟΧΙ» ΕΙΝΑΙ Η ΣΩΣΤΗ ΠΡΟΕΠΙΛΟΓΗ. Οι δύο απαντήσεις δεν κοστίζουν το
// ίδιο. Ένα λανθασμένο «όχι» σε υπαρκτή καρτέλα ρίχνει τον χρήστη στην
// Επισκόπηση: χάνει ένα κλικ και το βλέπει αμέσως. Ένα λανθασμένο «ναι» σε
// ανύπαρκτη τον αφήνει σε λευκή οθόνη που μοιάζει με χαλασμένη εφαρμογή, χωρίς
// τίποτα να του πει τι έγινε. Και το «όχι» είναι επαληθεύσιμο: ο κατάλογος από
// κάτω παράγεται από τη ΜΙΑ πηγή ονομάτων, οπότε καμία υπαρκτή καρτέλα δεν
// μπορεί να λείψει από αυτόν χωρίς να λείπει και το όνομά της από το μενού.
//
// Ο κατάλογος ΔΕΝ είναι τρίτος κανόνας ορατότητας — δεν λέει ποια καρτέλα
// φαίνεται, λέει ποια ΥΠΑΡΧΕΙ. Οι συνθήκες μένουν όπου ήταν.
const KNOWN_TABS: ReadonlySet<string> = new Set<string>([
  // Η μία πηγή ονομάτων καρτελών. Ό,τι έχει όνομα στο μενού, υπάρχει — μαζί με
  // τις «Επαφές», που δεν είναι προορισμός του μενού αλλά ενότητα μέσα στο
  // Αρχείο και προορισμός του βοηθού.
  ...Object.keys(NAV_LABELS),
  // Η Σύγκριση δεν έχει δική της θέση στο μενού: αποδίδεται μέσα στην Απόδοση.
  // Έχει όμως δικό της κανόνα εδώ (MIN_PROPERTIES) και ρωτιέται ονομαστικά,
  // οπότε είναι υπαρκτός κωδικός.
  'comparison',
  // ΨΕΥΔΟ-ΚΑΡΤΕΛΕΣ: δεν είναι οθόνες αλλά ΕΝΕΡΓΕΙΕΣ που ανοίγουν παράθυρο πάνω
  // από την τρέχουσα οθόνη — η σάρωση παραστατικού και η επεξεργασία στοιχείων
  // του ακινήτου. Τις στέλνουν η ατζέντα (βήμα ρύθμισης με nav 'edit') και η
  // μηχανή insights (κουμπί «Σάρωση»). Ισχύουν πάντα, ανεξάρτητα από κατάσταση.
  'scan', 'edit',
]);

/**
 * Καρτέλες που εξαρτώνται από την ΚΑΤΑΣΤΑΣΗ του ακινήτου.
 *
 * Ο κανόνας διαβάζεται σαν πρόταση: «ο Ενοικιαστής υπάρχει μόνο σε μακροχρόνια
 * μίσθωση». Ό,τι δεν αναφέρεται εδώ δεν εξαρτάται από κατάσταση.
 */
const BY_STATUS: Record<string, readonly PropertyStatus[]> = {
  tenant:  ['rent_long'],
  clients: ['rent_short'],
  pricing: ['rent_short'],
  // ΑΠΟΔΟΣΕΙΣ ΜΟΝΟ ΟΠΟΥ ΥΠΑΡΧΕΙ ΕΣΟΔΟ, ΟΧΙ ΟΠΟΥ ΘΑ ΜΠΟΡΟΥΣΕ ΝΑ ΥΠΑΡΞΕΙ.
  //
  // Ήταν ['rent_long','rent_short','vacant','for_sale'] με το σκεπτικό «όπου
  // μπορεί να υπάρξει έσοδο». Λάθος: σε κενό ή προς πώληση ακίνητο η απόδοση
  // δεν είναι μέτρηση, είναι υπόθεση — και μια υπόθεση με ποσοστό δίπλα της
  // διαβάζεται ως γεγονός. Το ίδιο ακριβώς πρόβλημα με τα επινοημένα νούμερα
  // που καθαρίστηκαν από όλο το app.
  //
  // Η ερώτηση «τι να το κάνω;» για τα ακίνητα που δεν αποδίδουν έχει δική της
  // απάντηση: την καρτέλα Σχέδιο, που συγκρίνει επιλογές με πραγματικούς άξονες
  // αντί για ένα ποσοστό χωρίς έσοδο από πίσω.
  //
  // Μένουν οι δύο καταστάσεις όπου το ακίνητο ΟΝΤΩΣ αποδίδει. Η εποχική
  // εκμετάλλευση δεν είναι τρίτη κατάσταση: το `readStatus` τη διαβάζει ως
  // rent_short, οπότε καλύπτεται εδώ.
  roi:     ['rent_long', 'rent_short'],
  // Ο εξοπλισμός καταγράφεται όταν τον χρησιμοποιεί κάποιος άλλος. Στο δικό
  // σου σπίτι ξέρεις τι έχεις.
  inventory: ['rent_long', 'rent_short'],
  // ΝΕΑ ΚΑΡΤΕΛΑ: μία, όχι τέσσερις. Το περιεχόμενο αλλάζει με την κατάσταση,
  // αλλά η θέση στο μενού μένει η ίδια — ο χρήστης μαθαίνει πού να κοιτάξει.
  plan:    ['vacant', 'disputed', 'for_sale', 'renovation'],
};

/** Καρτέλες που εξαρτώνται από τον ΑΡΙΘΜΟ των ακινήτων. */
const MIN_PROPERTIES: Record<string, number> = {
  // Το χαρτοφυλάκιο είναι εργαλείο για κάποιον που έχει χαρτοφυλάκιο. Με δύο
  // ακίνητα, η επισκόπηση του καθενός φτάνει και περισσεύει.
  portfolio: 3,
  // Η σύγκριση θέλει δύο, ΚΑΙ συγκρίσιμα. Ο δεύτερος όρος ελέγχεται χωριστά.
  comparison: 2,
};

// ── ΣΥΓΚΡΙΣΙΜΟΤΗΤΑ ─────────────────────────────────────────────────────────

/** Ο τύπος ακινήτου, κανονικοποιημένος. Κενό σημαίνει «δεν δηλώθηκε». */
export const normType = (s: unknown): string =>
  typeof s === 'string'
    ? s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/ς/g, 'σ').trim()
    : '';

export interface CompareGroup {
  /** Ο τύπος που ενώνει την ομάδα. */
  key: string;
  ids: string[];
  /** Προειδοποίηση όταν τα ακίνητα της ομάδας διαφέρουν ουσιωδώς. */
  warning: string | null;
}

/**
 * Πόσο διαφέρουν τα ακίνητα μιας ομάδας.
 *
 * ΓΙΑΤΙ ΠΡΟΕΙΔΟΠΟΙΟΥΜΕ ΑΝΤΙ ΝΑ ΚΡΥΒΟΥΜΕ: δύο διαμερίσματα είναι όντως
 * συγκρίσιμα ως κατηγορία. Αν όμως το ένα είναι 45 τ.μ. του 1975 στη Λάρισα και
 * το άλλο 130 τ.μ. του 2018 στη Γλυφάδα, η σύγκριση θα βγάλει «το δεύτερο
 * αποδίδει καλύτερα» — που είναι αληθές και εντελώς άχρηστο. Ο χρήστης έχει
 * δικαίωμα να τη δει· έχει και δικαίωμα να ξέρει τι κοιτάζει.
 *
 * Τα κατώφλια είναι συντηρητικά επίτηδες: προειδοποιούμε μόνο όταν η διαφορά
 * είναι τέτοια που αλλάζει το συμπέρασμα, όχι σε κάθε απόκλιση.
 */
function varianceWarning(items: readonly PropertyLike[]): string | null {
  const reasons: string[] = [];

  const sizes = items.map(p => Number(p.sqm)).filter(n => Number.isFinite(n) && n > 0);
  if (sizes.length >= 2) {
    const min = Math.min(...sizes), max = Math.max(...sizes);
    if (max / min >= 1.6) reasons.push(`μεγέθη από ${Math.round(min)} έως ${Math.round(max)} τ.μ.`);
  }

  const years = items.map(p => Number(p.year_built)).filter(n => Number.isFinite(n) && n > 1800);
  if (years.length >= 2) {
    const min = Math.min(...years), max = Math.max(...years);
    if (max - min >= 25) reasons.push(`έτη κατασκευής από ${min} έως ${max}`);
  }

  // Τα τρία πρώτα ψηφία του ΤΚ δείχνουν περιοχή. Διαφορετική περιοχή σημαίνει
  // διαφορετική αγορά και η αγορά κρίνει το ενοίκιο περισσότερο από το ακίνητο.
  const areas = new Set(items.map(p => String(p.postal_code ?? '').replace(/\D/g, '').slice(0, 3)).filter(Boolean));
  if (areas.size >= 2) reasons.push('διαφορετικές περιοχές');

  if (reasons.length === 0) return null;
  return `Τα ακίνητα διαφέρουν σημαντικά (${reasons.join(', ')}). Η σύγκριση μπορεί να οδηγήσει σε λανθασμένα συμπεράσματα.`;
}

/**
 * Οι ομάδες ακινήτων που συγκρίνονται μεταξύ τους.
 *
 * Ομαδοποίηση κατά ΤΥΠΟ. Ακίνητα χωρίς δηλωμένο τύπο ΔΕΝ μπαίνουν σε ομάδα:
 * το άγνωστο δεν είναι κατηγορία και μια ομάδα «χωρίς τύπο» θα έβαζε μαζί ένα
 * κατάστημα με μια αποθήκη επειδή κανένα από τα δύο δεν συμπληρώθηκε.
 */
export function comparableGroups(properties: readonly PropertyLike[]): CompareGroup[] {
  const byType = new Map<string, PropertyLike[]>();
  for (const p of properties) {
    const t = normType(p.prop_type);
    if (!t) continue;
    const list = byType.get(t);
    if (list) list.push(p); else byType.set(t, [p]);
  }
  return [...byType.entries()]
    .filter(([, items]) => items.length >= 2)
    .map(([key, items]) => ({
      key,
      ids: items.map(p => p.id),
      warning: varianceWarning(items),
    }))
    .sort((a, b) => b.ids.length - a.ids.length);
}

export const canCompare = (properties: readonly PropertyLike[]): boolean =>
  comparableGroups(properties).length > 0;

// ── Η ΑΠΟΦΑΣΗ ──────────────────────────────────────────────────────────────

export interface Decision {
  visible: boolean;
  /** Γιατί όχι, με λόγια. Κενό όταν φαίνεται. */
  reason: string;
  /**
   * ΑΦΟΡΑ ΚΑΝ ΑΥΤΟ ΤΟ ΑΚΙΝΗΤΟ;
   *
   * Δύο εντελώς διαφορετικοί λόγοι κρύβουν μια καρτέλα και μέχρι τώρα
   * συγχέονταν σε ένα `visible: false`:
   *
   *   · «Δεν το χρειάζεσαι ΑΚΟΜΗ» — λίγα ακίνητα για σύγκριση, δεν έχεις φτάσει
   *     ως εκεί. Προσωρινό. Με «δείξε τα όλα» έχει νόημα να φανεί αχνά, ώστε ο
   *     χρήστης να ξέρει ότι υπάρχει.
   *   · «ΔΕΝ ΙΣΧΥΕΙ για αυτό το ακίνητο» — ο Ενοικιαστής σε βραχυχρόνια, οι
   *     Αποδόσεις σε ιδιοχρησία. Δεν είναι «όχι ακόμη», είναι «ποτέ, όσο το
   *     ακίνητο είναι αυτό που είναι».
   *
   * Το «δείξε τα όλα» ανασταίνει το πρώτο. Το δεύτερο ΔΕΝ επιτρέπεται να το
   * αναστήσει: ένας ιδιοκτήτης βραχυχρόνιας που βλέπει «Ενοικιαστής» στο μενού
   * δεν βλέπει μια δυνατότητα, βλέπει λάθος.
   */
  applies: boolean;
}

/**
 * Φαίνεται αυτή η καρτέλα;
 *
 * Το `selected` μπορεί να είναι `null` (καμία επιλογή ακινήτου). Τότε οι
 * καρτέλες που εξαρτώνται από κατάσταση ΔΕΝ φαίνονται: δεν υπάρχει κατάσταση
 * να τις δικαιολογήσει και το να τις δείχναμε «προσωρινά» θα τις έκανε να
 * εξαφανίζονται μόλις ο χρήστης διαλέξει ακίνητο — η χειρότερη εμπειρία.
 */
/** Σήματα του ΕΠΙΛΕΓΜΕΝΟΥ ακινήτου που μπορούν να ανοίξουν μια κλειστή καρτέλα. */
export interface TabSignals {
  /** Έχει ήδη καταχωρημένα αντικείμενα σε αυτό το ακίνητο. */
  hasInventory?: boolean;
}

export function tabDecision(tabId: string, ctx: OwnerContext, selected: PropertyLike | null, signals: TabSignals = {}): Decision {
  // ── ΑΓΝΩΣΤΟΣ ΚΩΔΙΚΟΣ: ΟΧΙ, ΚΑΙ ΠΡΩΤΟΣ ΕΛΕΓΧΟΣ ────────────────────────────
  //
  // Μπαίνει πριν από κάθε άλλον κανόνα γιατί κανένας από τους επόμενους δεν
  // αφορά κάτι που δεν υπάρχει. Ο λόγος γράφεται με τον ίδιο τον κωδικό μέσα,
  // ΓΙΑ ΟΠΟΙΟΝ ΨΑΧΝΕΙ ΤΟ ΣΦΑΛΜΑ, ΟΧΙ ΓΙΑ ΤΗΝ ΟΘΟΝΗ: το tooltip της πλαϊνής
  // μπάρας δείχνει τη `reason` μόνο για κωδικούς των NAV_GROUPS και ένας
  // ανύπαρκτος κωδικός εξ ορισμού δεν είναι εκεί. Ο χρήστης βλέπει σκέτη την
  // Επισκόπηση· τη φράση τη διαβάζει η δοκιμή και όποιος διαβάζει τον κώδικα.
  //
  // `applies: false` και όχι `true`: το «δείξε τα όλα» της πλοήγησης ανασταίνει
  // ό,τι έχει `applies: true` (βλ. navVisible στη σελίδα). Με `true` εδώ, ένας
  // ανύπαρκτος κωδικός θα ξαναγινόταν ορατός μόλις ο χρήστης πατούσε «Όλες οι
  // καρτέλες» — δηλαδή η τρύπα θα έκλεινε μόνο για τους μισούς χρήστες.
  if (!KNOWN_TABS.has(tabId)) {
    return { visible: false, applies: false, reason: `Δεν υπάρχει καρτέλα με κωδικό «${tabId}».` };
  }

  // ── ΠΟΤΕ ΔΕΝ ΚΡΥΒΟΥΜΕ ΚΑΡΤΕΛΑ ΜΕ ΔΕΔΟΜΕΝΑ ΜΕΣΑ ────────────────────────────
  // Ο κανόνας «εξοπλισμός μόνο σε μίσθωση» είναι σωστός για ΚΕΝΗ καρτέλα. Για
  // γεμάτη είναι απώλεια: ο ιδιοκτήτης που κατέγραψε σαράντα αντικείμενα και
  // μετά άλλαξε την κατάσταση σε «Ιδιοχρησία» έχανε ΚΑΘΕ πρόσβαση σε αυτά — το
  // μενού δεν τα έδειχνε, η παλέτα εντολών τα φιλτράριζε, ο βοηθός τα
  // φιλτράριζε και το κουμπί του Αρχείου τον γύριζε στην Επισκόπηση.
  //
  // Το δίχτυ υπήρχε ήδη γραμμένο μέσα στην ίδια την καρτέλα («αν υπάρχουν ήδη
  // αντικείμενα, εμφανίζεται ΠΑΝΤΑ»), αλλά η καρτέλα δεν αποδιδόταν ποτέ για να
  // το εκτελέσει: η πλοήγηση την είχε ήδη κόψει. Ο κανόνας ανήκει εδώ, όπου
  // παίρνεται η απόφαση.
  if (tabId === 'inventory' && signals.hasInventory) {
    return { visible: true, applies: true, reason: '' };
  }

  const min = MIN_PROPERTIES[tabId];
  if (min !== undefined) {
    if (ctx.properties.length < min) {
      return {
        visible: false, applies: true,
        reason: min === 2
          ? 'Χρειάζονται τουλάχιστον δύο ακίνητα.'
          : `Εμφανίζεται από ${min} ακίνητα και πάνω.`,
      };
    }
    if (tabId === 'comparison' && !canCompare(ctx.properties)) {
      return { visible: false, applies: true, reason: 'Δεν υπάρχουν δύο ακίνητα ίδιου τύπου για σύγκριση.' };
    }
  }

  const statuses = BY_STATUS[tabId];
  if (statuses) {
    if (!selected) return { visible: false, applies: true, reason: 'Δεν έχει επιλεγεί ακίνητο.' };
    const s = readStatus(selected);
    if (!statuses.includes(s)) {
      // Η κατάσταση του ακινήτου το αποκλείει: δεν είναι «όχι ακόμη».
      return { visible: false, applies: false, reason: reasonForStatus(tabId, s) };
    }
  }

  return { visible: true, applies: true, reason: '' };
}

/**
 * Γιατί δεν φαίνεται, σε λόγια που εξηγούν αντί να απορρίπτουν.
 *
 * Η διατύπωση αποφεύγει το «δεν έχεις δικαίωμα» και λέει τι ισχύει: «το
 * ακίνητο είναι σε ιδιοχρησία». Ο χρήστης καταλαβαίνει ότι μπορεί να το
 * αλλάξει αν θέλει και ότι δεν του λείπει κάτι.
 */
function reasonForStatus(tabId: string, s: PropertyStatus): string {
  const what: Record<string, string> = {
    tenant: 'Ο ενοικιαστής',
    clients: 'Οι πελάτες',
    pricing: 'Η τιμολόγηση',
    roi: 'Οι αποδόσεις',
    inventory: 'Ο εξοπλισμός',
    plan: 'Το σχέδιο',
  };
  const state: Record<PropertyStatus, string> = {
    rent_long: 'σε μακροχρόνια μίσθωση',
    rent_short: 'σε βραχυχρόνια μίσθωση',
    vacant: 'κενό',
    own_use: 'σε ιδιοχρησία',
    renovation: 'σε ανακαίνιση',
    for_sale: 'προς πώληση',
    disputed: 'σε νομική εκκρεμότητα',
  };
  return `${what[tabId] ?? 'Αυτή η ενότητα'} δεν αφορά ακίνητο ${state[s]}.`;
}

// Το `visibleTabs`, που φιλτράρει έναν κατάλογο με το `tabDecision`, μετακόμισε
// στο visibility.testkit.ts. Δεν το καλούσε καμία οθόνη και, επειδή δεν δέχεται
// `signals`, θα έδινε ΑΛΛΗ απάντηση από τον πραγματικό καλών αν κάποιος το
// χρησιμοποιούσε νομίζοντας ότι είναι συντομογραφία του.

// ── ΟΙ ΕΝΟΤΗΤΕΣ ΤΗΣ ΛΟΓΙΣΤΙΚΗΣ ΚΡΙΝΟΝΤΑΙ ΜΕΣΑ ΣΤΗ ΛΟΓΙΣΤΙΚΗ ────────────────
//
// Εδώ ζούσαν ο τύπος `AccountingSection` και οι συναρτήσεις `accountingSections`
// και `accountingScope`: ποιες ενότητες αφορούν τον χρήστη και μια φράση που τις
// συνόψιζε. Δεν τις κάλεσε ποτέ καμία οθόνη. Το `accountingSections` φαινόταν
// χρησιμοποιημένο μόνο επειδή το καλούσε το `accountingScope` από κάτω του:
// δύο συναρτήσεις που κρατούσαν η μία την άλλη ζωντανή, χωρίς καταναλωτή.
//
// ΚΑΙ ΔΕΝ ΛΕΙΠΟΥΝ, ΓΙΑΤΙ Η ΑΠΑΝΤΗΣΗ ΔΙΝΕΤΑΙ ΗΔΗ ΑΛΛΟΥ. Το TabAccounting κρίνει
// με το δικό του `businessMode` (καθεστώς «Επιχείρηση (ΕΛΠ)» και μορφή): οι
// εισφορές ΕΦΚΑ, η προκαταβολή φόρου και οι αποσβέσεις είναι φραγμένες πίσω
// του, οπότε ο ιδιώτης δεν τις βλέπει. Και ο υπότιτλος της καρτέλας λέει ήδη τι
// δείχνει η οθόνη: «Επιχείρηση (ΕΛΠ)», «Βραχυχρόνια μίσθωση», «Μακροχρόνια
// μίσθωση».
//
// Οι δύο απαντήσεις δεν συμφωνούσαν κιόλας: το `businessMode` κοιτάζει το
// ΕΠΙΛΕΓΜΕΝΟ ακίνητο και το καθεστώς του, ενώ αυτές κοιτούσαν ΟΛΟ το
// χαρτοφυλάκιο και τη νομική μορφή. Σε μεικτό χαρτοφυλάκιο θα έδιναν
// διαφορετική εικόνα από αυτήν που βλέπει ο χρήστης.
//
// Ενας δεύτερος κριτής που κανείς δεν ρωτάει διαβάζεται ως κανόνας που ισχύει.
// Αν κάποτε η Λογιστική θελήσει έναν κριτή έξω από την οθόνη της, θα γραφτεί
// μαζί με τον καταναλωτή του, όχι πριν.

// ═══════════════════════════════════════════════════════════════════════════
// ΠΟΥ ΓΡΑΦΕΤΑΙ ΤΟ ΕΣΟΔΟ ΑΥΤΟΥ ΤΟΥ ΑΚΙΝΗΤΟΥ
// ─────────────────────────────────────────────────────────────────────────
// ΤΟ ΣΦΑΛΜΑ ΠΟΥ ΤΟ ΓΕΝΝΗΣΕ. Η κενή οθόνη της Λογιστικής είχε ως κύριο, μπλε
// κουμπί το «Καταχώρηση ενοικίου» και καλούσε πάντα την καρτέλα «tenant». Η
// Λογιστική όμως φαίνεται και στις επτά καταστάσεις, ενώ ο Ενοικιαστής μόνο
// στη μακροχρόνια μίσθωση. Σε βραχυχρόνια, κενό, ιδιοχρησία, ανακαίνιση, προς
// πώληση και νομική εκκρεμότητα ο χρήστης πατούσε το κύριο κουμπί και
// προσγειωνόταν στην Επισκόπηση, χωρίς λέξη εξήγησης: το `navSafe` της σελίδας
// ρίχνει σιωπηλά εκεί όποιον ζητήσει αόρατη καρτέλα.
//
// Η αντιστοιχία ζει ΕΔΩ, δίπλα στο `tabDecision` που κρίνει την ορατότητα, με
// δοκιμή που τα σταυρώνει: ό,τι επιστρέφει αυτή η συνάρτηση ΠΡΕΠΕΙ να είναι
// ορατό· όταν επιστρέφει `null`, καμία καρτέλα εσόδου δεν πρέπει να είναι.
// ═══════════════════════════════════════════════════════════════════════════

/** Οι δύο καρτέλες που δέχονται έσοδο. Καμία τους δεν είναι πάντα ορατή. */
export const INCOME_TABS = ['tenant', 'clients'] as const;

export interface IncomeEntry {
  /** Η καρτέλα που δέχεται την καταχώρηση. */
  tab: (typeof INCOME_TABS)[number];
  /** Το κουμπί. */
  label: string;
  /** Το ουσιαστικό μέσα σε πρόταση: «Καταχώρησε <ουσιαστικό> και έξοδα». */
  noun: string;
}

/**
 * Πού γράφεται το έσοδο, ή `null` όταν το ακίνητο δεν έχει έσοδο να γράψει.
 *
 * Το `null` ΔΕΝ είναι παράλειψη: κενό, ιδιοχρησία, ανακαίνιση, προς πώληση και
 * νομική εκκρεμότητα δεν αποφέρουν μίσθωμα. Εκεί η οθόνη προτείνει το έξοδο,
 * που είναι και η μόνη καρτέλα ορατή σε κάθε κατάσταση.
 */
export function incomeEntry(status: PropertyStatus): IncomeEntry | null {
  if (status === 'rent_long') return { tab: 'tenant', label: 'Καταχώρηση ενοικίου', noun: 'ενοίκια' };
  if (status === 'rent_short') return { tab: 'clients', label: 'Καταχώρηση διαμονής', noun: 'διαμονές' };
  return null;
}

