...
{
  for (const s of PLAN_STATUSES) {
    const text = prose(plan(s));
    ok(`${s}: κανένα ποσοστό στο περιεχόμενο`, !/\d+\s*%/.test(text));
    ok(`${s}: κανένα ποσό σε ευρώ`, !/\d+([.,]\d+)?\s*(€|ευρώ)/.test(text));
    ok(`${s}: καμία χρονολογία-προθεσμία`, !/\b(19|20)\d{2}\b/.test(text));
    ok(`${s}: χωρίς emoji`, !/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/u.test(text));
  }
