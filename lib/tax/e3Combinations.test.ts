...
// ΔΥΟ ΠΙΝΑΚΕΣ ΓΙΑ ΤΟ ΙΔΙΟ ΠΡΑΓΜΑ ΔΕΝ ΥΠΑΡΧΟΥΝ ΠΙΑ. Το myData.ts ονομάζει τους
// εννέα που παράγει η εφαρμογή, ΠΑΙΡΝΟΝΤΑΣ τα λεκτικά από εδώ· εδώ είναι και
// οι δεκαπέντε. Η σύγκριση των δύο θα ήταν πλέον μια τιμή με τον εαυτό της,
// άρα κενή. Εκείνο που μπορεί ακόμη να σπάσει είναι η αντιστοίχιση κλειδιού
// «2.4» → «category2_4», αν αλλάξει η γραφή σε μία από τις δύο πλευρές.
for (const c of Object.keys(EXPENSE_CLASS_LABEL) as ExpenseClass[]) {
  ok(!!EXPENSE_CLASS_LABEL_ALL[`category${c.replace('.', '_')}`], `το ${c} αντιστοιχεί σε κατηγορία του Ε3`);
  ok((EXPENSE_CLASS_LABEL[c] ?? '').trim().length > 3, `και παίρνει από εκεί το λεκτικό του`);
}
