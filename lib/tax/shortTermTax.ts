...
// ═══════════════════════════════════════════════════════════════════════════
// ΤΙ ΜΕΝΕΙ ΑΠΟ ΤΗ ΒΡΑΧΥΧΡΟΝΙΑ: ΟΙ ΓΡΑΜΜΕΣ ΚΑΙ ΤΟ ΑΘΡΟΙΣΜΑ ΤΟΥΣ
// ─────────────────────────────────────────────────────────────────────────
// ΓΙΑΤΙ ΖΕΙ ΕΔΩ ΚΑΙ ΟΧΙ ΣΤΗΝ ΟΘΟΝΗ. Η οθόνη διάλεγε μόνη της ποια μεγέθη θα
// δείξει και έπαιρνε το σύνολο από αλλού (`summary.net`). Δύο διαδρομές προς το
// ίδιο ποσό: το πλακίδιο του τέλους έδειχνε το ΟΦΕΙΛΟΜΕΝΟ ενώ αφαιρούνταν μόνο
// το ΑΚΑΛΥΠΤΟ, και το τέλος παρεπιδημούντων δεν είχε πλακίδιο καθόλου. Ο χρήστης
// πρόσθετε τη στήλη και έβγαζε άλλο νούμερο από το τυπωμένο.
//
// Τώρα οι γραμμές είναι ΔΕΔΟΜΕΝΑ και το σύνολο ΠΑΡΑΓΕΤΑΙ από αυτές. Καμία γραμμή
// δεν μπορεί να μετράει χωρίς να φαίνεται, και καμία να φαίνεται χωρίς να μετράει.
// ═══════════════════════════════════════════════════════════════════════════

/** Γραμμή της κατάστασης. Οι εκροές γράφονται ΘΕΤΙΚΕΣ και αφαιρούνται στο άθροισμα. */
export interface NetLine { key: string; label: string; amount: number; out: boolean }

/**
 * Οι γραμμές που φτάνουν στην οθόνη, με τη σειρά που διαβάζονται.
 *
 * Το τέλος ανθεκτικότητας μπαίνει με το ΑΚΑΛΥΠΤΟ μέρος του, γιατί μόνο αυτό
 * φεύγει από την τσέπη: ό,τι εισέπραξε ο οικοδεσπότης από τους επισκέπτες το
 * αποδίδει, δεν το πληρώνει. Το οφειλόμενο συνολικά λέγεται δίπλα, ως
 * συμφραζόμενο, όχι μέσα στη στήλη των αφαιρέσεων.
 */
export function shortTermNetLines(s: ShortTermYearSummary): NetLine[] {
  return [
    { key: 'gross', label: 'Δηλωτέα ακαθάριστα', amount: s.grossRevenue, out: false },
    { key: 'tax', label: 'Εκτιμώμενος φόρος εισοδήματος', amount: s.incomeTax, out: true },
    ...(s.levyShortfall > 0
      ? [{ key: 'levy', label: 'Τέλος ανθεκτικότητας που δεν κάλυψαν οι επισκέπτες', amount: s.levyShortfall, out: true }]
      : []),
    ...(s.municipalTax > 0
      ? [{ key: 'municipal', label: 'Τέλος παρεπιδημούντων', amount: s.municipalTax, out: true }]
      : []),
    ...(s.platformFees > 0
      ? [{ key: 'fees', label: 'Προμήθειες πλατφορμών', amount: s.platformFees, out: true }]
      : []),
  ];
}

/** Το υπόλοιπο, ΑΠΟ τις γραμμές. Η μόνη διαδρομή προς αυτό το ποσό. */
export function shortTermNet(lines: readonly NetLine[]): number {
  return Math.round(lines.reduce((sum, l) => sum + (l.out ? -l.amount : l.amount), 0) * 100) / 100;
}
