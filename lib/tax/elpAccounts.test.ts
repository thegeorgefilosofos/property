...
// ══ ΟΙ ΣΥΝΕΧΟΜΕΝΟΙ ΚΩΔΙΚΟΙ ΓΡΑΦΟΝΤΑΙ ΩΣ ΣΕΙΡΑ ══════════════════════════════
// Ο 71.04 αντιστοιχεί σε δεκαέξι κωδικούς ΕΓΛΣ. Γραμμένοι ένας ένας πιάνουν
// 110 χαρακτήρες και σπάνε σε δύο σειρές: ο λογιστής διαβάζει έναν τοίχο από
// αριθμούς για να βρει αν είναι μέσα ο δικός του.
console.log('\nΗ αντιστοιχία γράφεται για να διαβάζεται');
eq('τρεις και πάνω γίνονται σειρά', eglsOf('52'), '45.00 έως 45.03, 45.10 έως 45.13');
// ΔΥΟ ΔΕΝ ΓΙΝΟΝΤΑΙ ΣΕΙΡΑ. Το «62.00 έως 62.01» δεν είναι πιο σύντομο από το
// «62.00, 62.01» και είναι πιο δύσκολο να διαβαστεί.
eq('δύο μένουν δύο', eglsOf('64.02'), '62.00, 62.01');
eq('ένας μένει ένας', eglsOf('64.03'), '62.02');
eq('σειρά και μεμονωμένοι μαζί', eglsOf('64.11'), '63.02 έως 63.06, 63.98, 88.09');
// Η ΣΕΙΡΑ ΔΕΝ ΠΗΔΑΕΙ ΠΡΩΤΟΒΑΘΜΙΟ. Το 82.01 δεν είναι «επόμενο» του 81.03.
ok(!eglsOf('71.04').includes('81.03 έως 82.01'), 'δεν ενώνει διαφορετικούς πρωτοβάθμιους');
// ΚΑΙ ΚΑΝΕΝΑΣ ΚΩΔΙΚΟΣ ΔΕΝ ΧΑΝΕΤΑΙ ΣΤΗ ΣΥΜΠΤΥΞΗ: η σειρά «74.00 έως 74.02»
// πρέπει να περιέχει ΑΚΡΙΒΩΣ όσους έγραφε η λίστα.
{
  const expand = (s: string): string[] => s.split(', ').flatMap(part => {
    const m = /^(\d+)\.(\d+) έως (\d+)\.(\d+)$/.exec(part);
    if (!m) return part ? [part] : [];
    const out: string[] = [];
    for (let i = Number(m[2]); i <= Number(m[4]); i++) out.push(`${m[1]}.${String(i).padStart(2, '0')}`);
    return out;
  });
  const lost = ELP_ALL.filter(a => expand(eglsOf(a.code)).join(',') !== a.egls.join(','));
  eq('κάθε σειρά ξαναδίνει ακριβώς τους κωδικούς της', lost.map(a => a.code).join(', '), '');
}
