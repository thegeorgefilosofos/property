...
// Καθαρά = 1.500 − 213,75 − 7,50 = 1.278,75. Το ΤΑΚΚ (52) ΔΕΝ ξαναμπαίνει εδώ.
ok('net αφαιρεί τέλος παρεπ.', near(shortTermYearSummary(stays, 2026, { sqm: 60, propertyCount: 3 }).net, 1278.75));
ok('φόρος = κλίμακα(95% × 1500), τεκμαρτή έκπτωση 5%', near(sum.incomeTax, rentalIncomeTax(1500 * 0.95)) && near(sum.incomeTax, 213.75));
// Χωρίς τέλος παρεπ. (εξαίρεση): 1.500 − 213,75 = 1.286,25.
ok('καθαρά = μεικτά − φόρος (το ΤΑΚΚ είναι ήδη έξω από τα μεικτά)', near(sum.net, 1286.25) && near(sum.net, 1500 - sum.incomeTax));
