import { ELP_64 } from '../tax/elpAccounts';
...
// ─────────────────────────────────────────────────────────────────────────
// ΤΟ ΠΡΟΒΛΗΜΑ, ΓΡΑΜΜΕΝΟ ΚΑΘΑΡΑ. Οι παραπάνω κωδικοί είναι ΕΓΛΣ — το σχέδιο
// πριν από τα Ελληνικά Λογιστικά Πρότυπα. Το έντυπο Ε3 όμως, στο οποίο
// καταλήγουν αυτά τα ποσά, είναι χτισμένο πάνω στα ΕΛΠ (ν. 4308/2014): εκεί ο
// λογαριασμός των λειτουργικών εξόδων είναι ο 64 και όχι ο 62.
//
// ΓΙΑΤΙ ΔΕΝ ΑΛΛΑΞΑΝ ΟΙ ΚΩΔΙΚΟΙ. Το αρχείο που κατεβάζει ο λογιστής μπαίνει στο
// ΔΙΚΟ ΤΟΥ πρόγραμμα, με αντιστοιχίσεις που έχει ήδη αποθηκεύσει. Μια σιωπηλή
// αλλαγή του «62.03» σε «64.02» θα προσγειωνόταν στα βιβλία πελατών του χωρίς
// να το ζητήσει κανείς. Και πολλά λογιστήρια κρατούν ακόμη εσωτερικά το ΕΓΛΣ.
//
// ΓΙΑΤΙ ΔΕΝ ΕΜΕΙΝΕ ΜΟΝΟ ΤΟ ΕΓΛΣ. Επειδή τότε το αρχείο δεν λέει πουθενά πώς
// συνδέεται με το έντυπο που θα συμπληρωθεί — και η αντιστοιχία γίνεται στο
// κεφάλι κάποιου, μία φορά ανά γραμμή.
//
// Η ΑΠΑΝΤΗΣΗ ΕΙΝΑΙ ΚΑΙ ΤΑ ΔΥΟ, ΣΕ ΔΙΚΗ ΤΟΥ ΣΤΗΛΗ. Προστίθεται πληροφορία, δεν
// αφαιρείται καμία: όποιος δουλεύει με ΕΓΛΣ αγνοεί τη νέα στήλη, όποιος
// συμπληρώνει Ε3 τη διαβάζει. Η στήλη μπαίνει ΤΕΛΕΥΤΑΙΑ, γιατί οι αποθηκευμένες
// αντιστοιχίσεις των import wizards μετρούν θέσεις.
//
// ΤΑ ΟΝΟΜΑΤΑ ΕΙΝΑΙ ΤΟΥ ΝΟΜΟΥ (lib/tax/elpAccounts.ts, Παράρτημα Γ). Όπου δεν
// υπάρχει σωσμένη αντιστοιχία, το κελί μένει ΚΕΝΟ: το ταμείο, τα έσοδα και οι
// υποχρεώσεις δανείων έχουν δική τους αρίθμηση στα ΕΛΠ που δεν έχει διαβαστεί
// από επίσημη πηγή, και κωδικός χωρίς πηγή δεν γράφεται σε αρχείο λογιστή.
// ═══════════════════════════════════════════════════════════════════════════

/** ΕΓΛΣ → ΕΛΠ, μόνο όπου η αντιστοιχία είναι κυριολεκτική και σωσμένη. */
const EGLS_TO_ELP: Record<string, string> = {
  '62.01': '64.04', // Τηλεπικοινωνίες
  '62.02': '64.03', // Ύδρευση
  '62.03': '64.02', // Ενέργεια
  '62.04': '64.02', // Ενέργεια (αέριο, θέρμανση)
  '62.05': '64.06', // Ασφάλιστρα
  '62.07': '64.09', // Επισκευές και συντηρήσεις
  '62.98': '64.12', // Λοιπά έξοδα (κοινόχρηστα)
  '61.00': '64.01', // Αμοιβές για υπηρεσίες
  '63.00': '64.11', // Φόροι και τέλη (πλην φόρου εισοδήματος)
  '63.03': '64.11', // Φόροι και τέλη
  '64.98': '64.12', // Λοιπά έξοδα
  '65.00': '65',    // Χρεωστικοί τόκοι και συναφή έξοδα
};
...
export function journalCsvGeneric(lines: JournalLine[]): string {
  const head = ['Αρ.Άρθρου', 'Ημ/νία', 'Κωδικός', 'Περιγραφή Λογαριασμού', 'Αιτιολογία', 'Χρέωση', 'Πίστωση', 'Παραστατικό', 'ΑΦΜ/Αντισυμβ.', 'Κέντρο Κόστους', 'Λογαριασμός ΕΛΠ'];
