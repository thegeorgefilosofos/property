 ...
 * Προσπερνά κενά και σημεία στίξης στην αρχή («"Ο Γιώργος"», «(πρώην) ΔΕΗ»),
 * ώστε ένα εισαγωγικό να μη στέλνει την επαφή στο «#».
 */
export function alphaBucket(name: string | null | undefined): string {
  ...
  for (const ch of s) {
    ...
    // Ψηφίο ή σύμβολο στην αρχή: σταματάμε ΕΔΩ, δεν ψάχνουμε βαθύτερα.
    // Το «3G Telecom» ανήκει στο «#», όχι στο «G» — αλλιώς δεν θα το έβρισκε
    // κανείς ούτε στο ένα ούτε στο άλλο.
    return OTHER_BUCKET;
  }
  return OTHER_BUCKET;
}
