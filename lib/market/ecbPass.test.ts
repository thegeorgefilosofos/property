...
async function main() {
  // ── Η ΣΕΙΡΑ ΤΩΝ ΥΠΟΨΗΦΙΩΝ ────────────────────────────────────────────────
  // Η ημερήσια σειρά δεν απαντά· η μηνιαία απαντά. Η τιμή γράφεται, αλλά ΜΕ ΤΗ
  // ΔΙΚΗ ΤΗΣ ΒΑΣΗ: «μέσος όρος μήνα», όχι «ημερήσιο κλείσιμο». Αν το χάναμε
  // αυτό, η οθόνη θα παρουσίαζε μέσο όρο Ιουνίου ως σημερινό κλείσιμο.
  const daily = async (url: string) => url.includes('/FM/D.') ? bad(404) : ok(2.3397, '2026-06');
  const r1 = await fetchLatest(NOW, daily as never);
  eq('η μηνιαία σειρά καλύπτει την ημερήσια', r1.fresh.euribor_3m?.value, 2.3397);
  eq('και η βάση μέτρησης λέει την αλήθεια',  r1.fresh.euribor_3m?.basis, 'μέσος όρος μήνα');
  eq('με την ημερομηνία της παρατήρησης',     r1.fresh.euribor_3m?.asOf, '2026-06-01');
