...
export function computeInsights(input: InsightInput): Insight[] {
  ...
  if (netYield > 0 && propValue > 0) {
    if (netYield >= 5) out.push({ id: 'yield-strong', kind: 'positive', title: 'Δυνατή απόδοση', detail: `Το ακίνητο σου αποδίδει καθαρά ${netYield.toFixed(1)}% τον χρόνο, πάνω από τον μέσο όρο της αγοράς. Για σύγκριση, μια τυπική απόδοση χρηματιστηρίου είναι γύρω στο 7% αλλά με ρίσκο και μεταβλητότητα.`, metric: `${netYield.toFixed(1)}%` });
    else if (netYield < 3) out.push({ id: 'yield-low', kind: 'opportunity', title: 'Υπάρχει περιθώριο στην απόδοση', detail: `Η καθαρή απόδοση είναι ${netYield.toFixed(1)}%. Δες αν το ενοίκιο είναι κάτω από την αγορά ή αν κάποιες δαπάνες μειώνονται. Μικρές αλλαγές ανεβάζουν αισθητά την απόδοση.`, metric: `${netYield.toFixed(1)}%`, action: { label: 'Αποδόσεις', tab: 'roi' } });
  }
