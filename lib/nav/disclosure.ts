...
export interface DisclosureSignals {
  ...
  isShortTerm?: boolean;
  /** Το επιλεγμένο ακίνητο ΑΠΟΔΙΔΕΙ: μακροχρόνια, βραχυχρόνια ή εποχικά. */
  isRented?: boolean;
...
const SIGNAL_RULES: Record<string, (s: DisclosureSignals) => boolean> = {
  ...
  // Σύγκριση: χρειάζεται δεύτερο ακίνητο. Ένα ακίνητο δεν συγκρίνεται με τίποτα.
  comparison: s => (s.propertyCount ?? 0) > 1,
  // ΑΠΟΔΟΣΕΙΣ: ΜΟΝΟ κριτήριο είναι αν το ακίνητο αποδίδει.
  //
  // Ήταν `propertyCount > 1`, που σήμαινε ότι ο ιδιοκτήτης ΕΝΟΣ νοικιασμένου
  // διαμερίσματος δεν έβλεπε ποτέ την απόδοσή του — ενώ είναι ακριβώς το νούμερο
  // για το οποίο έψαξε εργαλείο. Το πλήθος ακινήτων δεν έχει καμία σχέση με το
  // αν ένα μίσθωμα αποδίδει· αυτό ήταν κανόνας της Σύγκρισης που είχε αντιγραφεί
  // εδώ από παραδρομή.
  //
  // Η κατάσταση κρίνεται σε ΕΝΑ σημείο (lib/property/visibility.ts): εκεί οι
  // Αποδόσεις υπάρχουν μόνο σε rent_long και rent_short. Εδώ απαντάμε άλλο
  // ερώτημα — «αξίζει να εμφανιστεί ήδη;» — και η απάντηση είναι ναι, μόλις
  // υπάρξει μίσθωση.
  roi:        s => !!s.isRented,
