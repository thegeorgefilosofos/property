...
import { PLANS, normalizePlan, TRIAL_DAYS, type PlanId } from './plans';
...
//
// ΤΟ ΣΦΑΛΜΑ, ΟΠΩΣ ΗΤΑΝ ΓΡΑΜΜΕΝΟ. Το ταμείο ζητούσε `trial_period_days: 30`
// σε ΚΑΘΕ αγορά. Δηλαδή όποιος είχε ήδη ξοδέψει τη δοκιμή του μέσα στην
// εφαρμογή έπαιρνε άλλες τριάντα ημέρες δωρεάν μόλις πάταγε «Πληρωμή» — και
// οι Οροι έγραφαν το αντίθετο: «με τη λήξη της δοκιμής, ο λογαριασμός μένει
// σε αναμονή μέχρι να επιλέξεις πακέτο και να πληρώσεις».
//
// Η δοκιμή δεν ξεκινά στο ταμείο, ξεκινά στην ΕΓΓΡΑΦΗ. Εδώ υπολογίζεται πόση
// έχει απομείνει: όποιος αγοράζει μέσα στη δοκιμή του δεν χάνει τις ημέρες
// που δεν πρόλαβε, και όποιος αγοράζει μετά πληρώνει αμέσως.
//
// ΤΟ ΟΡΙΟ ΤΩΝ ΔΥΟ ΗΜΕΡΩΝ ΕΙΝΑΙ ΤΟΥ ΠΑΡΟΧΟΥ, όχι δικό μας: η Stripe απορρίπτει
// `trial_end` που απέχει λιγότερο από 48 ώρες. Χωρίς αυτόν τον έλεγχο, μια
// αγορά την τελευταία ημέρα της δοκιμής θα απορριπτόταν ολόκληρη.
const DAY_MS = 24 * 60 * 60 * 1000;
export const MIN_TRIAL_MS = 2 * DAY_MS;

/**
 * Η στιγμή που λήγει η δοκιμή, σε δευτερόλεπτα Unix, ή `null` για άμεση
 * χρέωση. Το `null` είναι η ΑΣΦΑΛΗΣ απάντηση: άγνωστη ημερομηνία εγγραφής δεν
 * χαρίζει μήνα.
 */
export function trialEndFor(createdAt: unknown, now: number, days = TRIAL_DAYS): number | null {
  ...
  const ends = born + days * DAY_MS;
  if (ends - now < MIN_TRIAL_MS) return null;
