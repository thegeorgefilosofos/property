...
async function asyncChecks() {
  ...
  // ── ΤΙ ΕΠΙΤΡΕΠΕΤΑΙ ΝΑ ΓΡΑΨΕΙ Ο ΠΕΛΑΤΗΣ ─────────────────────────────────
  {
    ...
    // Η ΣΦΡΑΓΙΔΑ ΔΕΝ ΤΑΞΙΔΕΥΕΙ ΑΠΟ ΤΟΝ ΠΕΛΑΤΗ. Την έγραφε το στρώμα με
    // `new Date()`, δηλαδή με το ρολόι του περιηγητή: σε συσκευή με λάθος ώρα η
    // ταξινόμηση «τελευταία αλλαγή πρώτη» έβγαζε άλλη σειρά. Τη γράφει πλέον η
    // σκανδάλη `billing_profiles_updated_at` (μετανάστευση 20260819170000), με
    // τον χρόνο του διακομιστή.
    ok('η σφραγίδα χρόνου δεν στέλνεται από τον πελάτη', !('updated_at' in row));
  }
  {
