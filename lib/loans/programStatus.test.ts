...
// ══ Η ΤΕΛΕΥΤΑΙΑ ΜΕΡΑ ΤΟΥ «ΣΠΙΤΙ ΜΟΥ ΙΙ» ═══════════════════════════════════
// Στις 31/08/2026 η οθόνη έγραφε «Λήγει σύντομα · Οι αιτήσεις κλείνουν, σε
// λιγότερο από μία μέρα» για πρόγραμμα του οποίου οι αιτήσεις είχαν κλείσει
// στις 31/05. Δύο ξεχωριστά σφάλματα, δύο ξεχωριστοί έλεγχοι.
{
  const spiti = { applicationDeadline: '31/05/2026', deadline: '31/08/2026', status: 'active' };
  const at = (iso: string) => programStatus(spiti, new Date(`${iso}T12:00:00`));

  eq('την τελευταία μέρα υπογραφής, οι αιτήσεις έχουν ήδη κλείσει',
    at('2026-08-31').acceptsApplications, false);
  eq('και το λέει με τη σωστή ημερομηνία',
    at('2026-08-31').note,
    'Οι αιτήσεις έκλεισαν 31/05/2026. Όσοι έχουν ήδη έγκριση υπογράφουν έως 31/08/2026.');
  eq('από την επομένη το πρόγραμμα έχει κλείσει',
    at('2026-09-01').state, 'closed');

  // Χωρίς προθεσμία αίτησης, η πρόταση δεν κρεμάει κόμμα στο κενό.
  const noApply = programStatus({ applicationDeadline: '', deadline: '31/08/2026', status: 'active' },
    new Date('2026-08-31T12:00:00'));
  eq('κενή προθεσμία αίτησης δεν αφήνει κόμμα να κρέμεται',
    noApply.note, 'Οι αιτήσεις κλείνουν 31/08/2026, σε λιγότερο από μία μέρα.');
}
