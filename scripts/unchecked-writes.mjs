...
for (const f of files()) {
  ...
  while ((m = FROM.exec(src)) !== null) {
    ...
    // Η ΓΕΙΤΝΙΑΣΗ ΔΕΝ ΕΙΝΑΙ ΚΑΛΥΨΗ.
    // Οι προηγούμενες γραμμές μετρούσαν ΠΑΝΤΑ, οπότε ένα `saved(...)` δύο
    // γραμμές πιο πάνω «κάλυπτε» ένα εντελώς ΑΛΛΟ γράψιμο από κάτω του. Έτσι
    // πέρασε απαρατήρητη μια διαγραφή φθοράς ακριβώς κάτω από δύο ελεγμένες
    // εγγραφές. Τώρα η αναδρομή σταματά στο τέλος της προηγούμενης πρότασης:
    // μετράνε μόνο γραμμές που ΣΥΝΕΧΙΖΟΝΤΑΙ σε αυτήν εδώ.
    const prev = [];
    for (let i = ln - 2; i >= Math.max(0, ln - 1 - LOOKBACK); i--) {
