...
const EXEMPT = {
  ...
  // Ελέγχθηκε γραμμή προς γραμμή: διαβάζει `bills` ΜΟΝΟ για να παραγάγει
  // εγγραφές `calendar_events` (συγχρονισμός προθεσμιών) και γράφει μία δαπάνη
  // όταν ένα γεγονός έχει κόστος. Τα δύο αθροίσματά του (ποσό μήνα, ποσό ημέρας)
  // τρέχουν πάνω σε `calendar_events`, όχι σε λογαριασμούς/δαπάνες — δεν παράγει
  // διασταυρούμενο σύνολο και δεν μπορεί να διπλομετρήσει τους δύο πίνακες.
  'app/dashboard/components/TabCalendar.tsx': 'Συγχρονίζει προθεσμίες σε calendar_events· τα σύνολά του είναι πάνω σε γεγονότα, όχι σε λογαριασμούς/δαπάνες.',
};
