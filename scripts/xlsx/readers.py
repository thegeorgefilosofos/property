...
def main(out):
    ...
    # ═══ Η ΤΥΦΛΗ ΑΝΑΓΝΩΣΤΡΙΑ ΚΑΤΗΓΟΡΟΥΣΕ ΤΟ ΠΡΟΪΟΝ ══════════════════════════
    # Η openpyxl ΔΕΝ διαβάζει εικόνες χωρίς την Pillow: το `ws._images` βγαίνει
    # άδειο, σιωπηλά, χωρίς προειδοποίηση. Ο έλεγχος από κάτω το διάβαζε ως
    # «το φύλλο δεν έχει σήμα» και ανέφερε είκοσι ένα φύλλα χαλασμένα σε έξι
    # βιβλία. Το `unzip -l` έδειχνε το `xl/media/properwise-mark.png` μέσα σε
    # ΚΑΘΕ ένα από αυτά: τα βιβλία ήταν σωστά, η αναγνώστρια ήταν τυφλή.
    #
    # ΓΙ' ΑΥΤΟ ΣΚΑΕΙ ΚΑΙ ΔΕΝ ΠΑΡΑΛΕΙΠΕΤΑΙ. Ενας έλεγχος που δεν μπορεί να δει
    # έχει δύο τρόπους να πει ψέματα: να κατηγορήσει (ό,τι έκανε) ή να πει
    # «πράσινο» για κάτι που δεν κοίταξε. Το τρίτο —«δεν μπορώ να κρίνω, βάλε
    # μου μάτια»— είναι το μόνο τίμιο.
    try:
        import PIL  # noqa: F401
    except ImportError:
        print('  ✗ Η openpyxl δεν βλέπει εικόνες χωρίς την Pillow: κάθε φύλλο θα')
        print('    έβγαινε «χωρίς σήμα» ενώ το σήμα είναι μέσα στο αρχείο.')
        print('    Εγκατάστησέ την:  pip install Pillow')
        return 1
