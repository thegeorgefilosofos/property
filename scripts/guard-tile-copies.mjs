...
// ── ΤΟ ΣΧΗΜΑ ΔΕΝ ΚΡΙΝΕΤΑΙ ΑΠΟ ΚΕΙΜΕΝΟ, ΚΑΙ ΤΟ ΔΟΚΙΜΑΣΑ ─────────────────────
// Ο φύλακας έχασε έξι χειρόγραφες γραφές επειδή δεν χρησιμοποιούν `kpi-card`:
// γράφουν δικό τους κουτί από το μηδέν. Δοκιμάστηκε ευρετικό πάνω στο κείμενο
// («ετικέτα κεφαλαίων και, μέσα σε δύο γραμμές, νούμερο με ρητό μέγεθος») και
// έβγαλε 35 ευρήματα· με κατώφλι μεγέθους, 17. Τα περισσότερα ήταν επικεφαλίδες
// ενότητας πάνω από πίνακα, δηλαδή ΟΧΙ πλακίδια. Ενας φύλακας με τόσα ψευδή
// ευρήματα μαθαίνει τον κόσμο να τον προσπερνά.
//
// Η ερώτηση θέλει ΑΠΟΔΟΣΗ: ποιο κείμενο ΦΑΙΝΕΤΑΙ ως ετικέτα και ποιο ΦΑΙΝΕΤΑΙ
// ως νούμερο, στην πραγματική οθόνη. Ζει στο `scripts/e2e-tile-book.mjs`, που
// ανοίγει τις σκηνές και μετράει. Εδώ μένει ό,τι κρίνεται με βεβαιότητα από το
// κείμενο: το κουτί και η γραμμή του βιβλίου, γραμμένα με τα ονόματά τους.
...
for (const f of projectFiles()) {
  ...
  lines.forEach((l, i) => {
    if (CARD.test(l)) { hits.push([f, i + 1, l, 'πλακίδιο: <Tile>']); return }
    if (PLAIN.test(l) && plain && label) hits.push([f, i + 1, l, 'γραμμή στοιχείων: <Stat>']);
  });
}
