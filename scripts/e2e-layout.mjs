...
const PROBE = () => {
  ...
  // ══════════════════════════════════════════════════════════════════════
  // ΜΗΚΟΣ ΓΡΑΜΜΗΣ: Ο ΜΟΝΟΣ ΑΝΙΧΝΕΥΤΗΣ ΠΟΥ ΡΩΤΑΕΙ ΑΝ ΤΟ ΚΕΙΜΕΝΟ ΔΙΑΒΑΖΕΤΑΙ
  // ──────────────────────────────────────────────────────────────────────
  ...
  // τις ερωτήσεις δεν πιάνει μια υποσημείωση που απλώνεται σε ολόκληρο το
  // πλάτος μιας φαρδιάς κάρτας, γιατί τεχνικά δεν φταίει τίποτα.
  //
  // ΤΙ ΜΕΤΡΗΘΗΚΕ ΠΡΙΝ ΓΡΑΦΤΕΙ. Σε δεκαεπτά σκηνές επί τέσσερα πλάτη βρέθηκαν
  // πέντε τέτοια μπλοκ: 143 χαρακτήρες ανά γραμμή στην Τιμολόγηση, 134 στη
  // Σύγκριση, 110 στις Αποδόσεις, 92 και 88 σε δύο ακόμη. Πάνω από περίπου
  // ενενήντα, το μάτι χάνει τη σειρά γυρίζοντας αριστερά και ξαναδιαβάζει την
  // ίδια. Το όριο της τυπογραφίας είναι 45 ώς 75· τα 143 είναι σχεδόν διπλά.
  //
  // ΚΑΙ ΦΑΙΝΟΤΑΝ ΜΟΝΟ ΣΤΗΝ ΤΑΜΠΛΕΤΑ. Στα 768 ώς 1024 η κάρτα είναι ήδη φαρδιά
  // ενώ οι στήλες δεν έχουν σπάσει ακόμη σε δύο. Στο κινητό περιορίζει η οθόνη,
  // στον φορητό το σπάσιμο σε στήλες. Η ζώνη της ταμπλέτας ήταν απροστάτευτη.
  //
  // ΤΟ ΟΡΙΟ ΕΙΝΑΙ 95 ΚΑΙ ΟΧΙ 85. Η διόρθωση φέρνει τα μπλοκ γύρω στους 84 με
  // το `.text-measure`· ένα όριο κολλητά από πάνω θα κοκκίνιζε με την πρώτη
  // αλλαγή γραμματοσειράς. Τα 95 είναι το σημείο όπου η ανάγνωση χαλάει
  // πραγματικά, δηλαδή ελάττωμα και όχι προτίμηση.
  //
  ...
  for (const el of document.querySelectorAll('p, li, div, span')) {
    ...
    if (perLine > 95) add('ΓΡΑΜΜΗ ΠΟΛΥ ΜΑΚΡΙΑ', t, `${perLine} χαρακτήρες`)
  }
...
const SCENES = ['portfolio','cash','rent','inbox','ledger','checklist','modal','select','compare','loan','pricing','bills','contacts','wizard','roi','tenant','scan','accounting','calendar','clients','documents']
