 ...
 *
 *     verify                        6΄53΄΄ →  7
 *     Σαρώσεις περιηγητή           10΄47΄΄ → 11
 *     Ο,τι κρίνεται αφού ζωγραφιστεί 13΄33΄΄ → 14
 *     Ολα πράσινα                   3΄΄    →  1  (δάπεδο ενός λεπτού)
 *
 * Η ΔΙΑΤΑΞΗ ΞΑΝΑΜΕΤΡΗΘΗΚΕ ΟΤΑΝ ΜΕΓΑΛΩΣΕ Ο ΚΑΤΑΛΟΓΟΣ ΣΥΣΚΕΥΩΝ. Ηταν 5΄54΄΄ με
 * δώδεκα· με είκοσι μία —τα πραγματικά πλάτη iPhone και iPad που έλειπαν— είναι
 * 9΄40΄΄ στο CI για δεκαοκτώ και 10΄40΄΄ τοπικά για είκοσι μία. Στρογγυλεύεται
 * στα 12, με περιθώριο: ο δρομέας δεν έχει πάντα την ίδια διάθεση.
 *
 *     Διάταξη σε είκοσι μία συσκευές        → 12
 *                                            ───
 *                                             45 λεπτά ανά εκτέλεση
 *
 * ΤΟ ΚΟΣΤΟΣ ΕΙΝΑΙ ΠΡΑΓΜΑΤΙΚΟ ΚΑΙ ΑΞΙΖΕΙ. Οι έξι νέες συσκευές iPhone βρήκαν
 * σφάλμα με την πρώτη εκτέλεση, σε πλάτος που δεν είχε κοιταχτεί ποτέ.
 */
const CI_MINUTES_PER_RUN = 45
