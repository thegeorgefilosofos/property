-- ═══════════════════════════════════════════════════════════════════════
-- ⚠️  DEPRECATED — ΜΗΝ ΤΟ ΤΡΕΧΕΙΣ ΠΛΕΟΝ ΜΕ ΤΟ ΧΕΡΙ.
--
-- Αυτό το αρχείο ήταν το παλιό «τρέξε τα όλα στον SQL Editor» setup. Πλέον το
-- σχήμα χτίζεται αποκλειστικά από τα versioned migrations στο
-- supabase/migrations/ (baseline 00000000000000_baseline.sql + companions),
-- τα οποία εφαρμόζονται ΑΥΤΟΜΑΤΑ από το CI/CD (.github/workflows/supabase-deploy.yml).
-- Ένα φρέσκο project ξαναχτίζεται ολόκληρο από τα migrations μόνο (finding #16).
--
-- Κρατιέται μόνο ως ιστορικό αναφοράς. Για setup νέου περιβάλλοντος:
--   supabase db push   (ή απλώς push στο branch — το τρέχει το pipeline)
-- Μην το τρέχεις χειροκίνητα: θα αποκλίνει από το migration history.
-- ═══════════════════════════════════════════════════════════════════════
