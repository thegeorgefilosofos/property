...
// mis-gendered. Callers prepend the leading space only for the gendered forms.
const toTenant = (c: Personal) => gv(c.tenantGender, { m: ' στον ενοικιαστή', f: ' στην ενοικιάστρια', n: '' });
...
export const MSG: Record<string, (c: Personal) => ChannelMessage> = {
  ...
  tax_enfia: (c) => ({ title: 'Πλησιάζει ο ΕΝΦΙΑ', body: 'Δες τι αναλογεί σε κάθε ακίνητό σου και τις δόσεις που έρχονται, χωρίς εκπλήξεις.', cta: 'Δες τον ΕΝΦΙΑ' }),
  enfia_installment_reminder: (c) => ({ title: 'Λήγει δόση ΕΝΦΙΑ', body: `${(c.installmentNo && c.installmentsTotal) ? `Η δόση ${c.installmentNo}/${c.installmentsTotal} λήγει` : 'Μια δόση λήγει'}${c.deadlineDate ? ` στις ${c.deadlineDate}` : ' σύντομα'}. Δες το ποσό ανά ακίνητο εγκαίρως.`, cta: 'Δες τις δόσεις' }),
  ...
  maintenance_requested: (c) => ({ title: 'Νέο αίτημα από το ακίνητό σου', body: 'Μπήκε ένα νέο αίτημα επισκευής και περιμένει την απάντησή σου. Δες τι χρειάζεται όσο είναι νωρίς· τα μικρά θέματα λύνονται πιο εύκολα.', cta: 'Δες και απάντησε' }),
  takk_seasonal_rate_switch: (c) => ({ title: 'Αλλάζει το τέλος ανθεκτικότητας', body: 'Από σήμερα ισχύει νέα εποχική χρέωση. Ενημέρωσε τις χρεώσεις των κρατήσεών σου, ώστε να εισπράττεις το σωστό ποσό σε κάθε αναχώρηση.', cta: 'Δες τις νέες χρεώσεις' }),
  ...
  // ── Short-term ops ─────────────────────────────────────────────────────────
  // Privacy: the guest's name stays behind the tap (a lock screen is public); the
  // glance shows only the property and the checklist prompt.
  checkin_today: (c) => ({ title: 'Άφιξη σήμερα', body: `${c.propertyName ? c.propertyName + ': άφιξη' : 'Έχεις άφιξη'} σήμερα. Κλειδιά, κωδικός και οδηγίες έτοιμα;`, cta: 'Δες την κράτηση' }),
