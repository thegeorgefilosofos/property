...
// Δεκαέξι προγράμματα, 109 emails: 1) Onboarding 2) Engagement 3) Upsell
  ...
  welcome_free: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Καλωσόρισες') + h('Ας ξεκινήσουμε') + greeting(c.name)
        + p('Από σήμερα, εσύ καταχωρείς τα βασικά και το Property OS κάνει τα υπόλοιπα: έσοδα, έξοδα, φόρους και υπενθυμίσεις, σε ένα μέρος.')
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Φωνητικές εντολές') + h('Μίλα, και γίνεται') + greeting(c.name)
        ...
        + bullets([
            ...
            'Ρώτησε ό,τι θες και πάρε απάντηση με τα δικά σου νούμερα.',
          ])
  ...
  product_update: (c) => {
    const detail = c.summaryText
      ...
      : bullets([
          ...
          'Εξυπνότερος βοηθός, που απαντά με τα δικά σου νούμερα.',
          'Λιγότερα βήματα στην καθημερινή καταχώρηση, με φωνητικές εντολές.',
        ])
    return { subject: 'Τι νέο έχει το Property OS', html: emailShell({
  ...
  tax_e2: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Φορολογικό ημερολόγιο') + h('Ώρα για το Ε2') + greeting(c.name)
        + p(`Η προθεσμία για τη δήλωση εισοδήματος${c.deadlineDate ? ` (έως ${esc(c.deadlineDate)})` : ''} πλησιάζει. Το Property OS συγκεντρώνει τα ενοίκιά σου ανά ακίνητο και ετοιμάζει το Ε2 σου προσυμπληρωμένο, με τα ποσά στη σχετική στήλη, έτοιμο για τον λογιστή σου ή για το myAADE.`)
        + p('Επισημαίνει και όσα λείπουν, ώστε να μην τρέχεις την τελευταία στιγμή. Τα νούμερα είναι ήδη στη θέση τους.')
...
export const OPERATIONS: Record<string, CopyFn> = {
  ...
  lease_ending: (c) => {
    ...
    return { subject: c.propertyName ? `Λήγει η μίσθωση του «${esc(c.propertyName)}»` : 'Πλησιάζει λήξη μίσθωσης', html: emailShell({
      ...
      bodyHtml: eyebrow('Μίσθωση') + h('Μια μίσθωση πλησιάζει στη λήξη της') + greeting(c.name)
        ...
        + p('Το Property OS κρατά τους όρους, τις ημερομηνίες και το ιστορικό έτοιμα, ώστε να κινηθείς χωρίς τρέξιμο.')
  ...
  lease_renewal_prompt: (c) => {
    ...
    return { subject: 'Ώρα για ανανέωση μίσθωσης;', html: emailShell({
      ...
      bodyHtml: eyebrow('Ανανέωση') + h('Ανανέωσε με σιγουριά') + greeting(c.name)
        + p(`Αν σκέφτεσαι να ανανεώσεις τη μίσθωση${ref}, το Property OS σε βοηθά να υπολογίσεις την αναπροσαρμογή του ενοικίου με βάση τα ισχύοντα όρια και ετοιμάζει ένα σχέδιο ειδοποίησης προς ${gv(c.tenantGender, { m: 'τον ενοικιαστή', f: 'την ενοικιάστρια', n: 'τον μισθωτή' })}, έτοιμο να το προσαρμόσεις και να το στείλεις.`)
  ...
  certificate_expiring: (c) => {
    const name = c.certificateName ? esc(c.certificateName) : 'πιστοποιητικό';
    const ref = c.propertyName ? ` για το «${esc(c.propertyName)}»` : '';
    const when = c.certificateEndDate ? ` στις <b>${esc(c.certificateEndDate)}</b>` : ' σύντομα';
    return { subject: `Λήγει το ${name}`, html: emailShell({
      ...
      bodyHtml: eyebrow('Πιστοποιητικά') + h('Ένα πιστοποιητικό θέλει ανανέωση') + greeting(c.name)
        + p(`Το ${name}${ref} λήγει${when}. Αν σχεδιάζεις μίσθωση, πώληση ή ενεργειακή αναβάθμιση, καλό είναι να το ανανεώσεις εγκαίρως, ώστε να μη σε καθυστερήσει.`)
  ...
  occupancy_gap: (c) => {
    ...
    return { subject: 'Κενές νύχτες, ευκαιρία για πλήρωση', html: emailShell({
      ...
      bodyHtml: eyebrow('Πληρότητα') + h('Υπάρχει χώρος για πλήρωση') + greeting(c.name)
        ...
        + p('Το Property OS σου προτείνει τιμή με βάση τη ζήτηση και τη σεζόν.')
...
export const PRODUCT: Record<string, CopyFn> = {
  ...
  assistant_showcase: (c) => {
    ...
    return { subject: 'Ο βοηθός σου κάνει τη δουλειά', html: emailShell({
      ...
      bodyHtml: eyebrow('Ο βοηθός σου') + h('Πες του τι θες, το κάνει') + greeting(c.name)
        ...
        + bullets([
            '«Πόσο φόρο θα πληρώσω φέτος;» και απαντά με τα δικά σου νούμερα.',
  ...
  changelog_monthly: (c) => {
    const body = c.summaryText ? p(esc(c.summaryText)) : p('Αυτόν τον μήνα προσθέσαμε νέες δυνατότητες, φτιάξαμε λεπτομέρειες και κάναμε το Property OS πιο γρήγορο. Όλα με έναν στόχο: να σου γλιτώνουμε χρόνο.');
    return { subject: 'Τι φτιάξαμε αυτόν τον μήνα', html: emailShell({
      ...
      bodyHtml: eyebrow('Ενημέρωση προϊόντος') + h('Ένας μήνας βελτιώσεων') + greeting(c.name)
        + p('Δουλεύουμε κάθε μέρα ώστε το Property OS να γίνεται καλύτερο. Να τι άλλαξε τον τελευταίο μήνα:')
        + body
        + button('Δες τι νέο υπάρχει', dash(c))
  ...
  roadmap_preview: (c) => {
    ...
    return { subject: 'Τι ετοιμάζουμε στη συνέχεια', html: emailShell({
      ...
      bodyHtml: eyebrow('Τι έρχεται') + h('Μια ματιά στο μέλλον του Property OS') + greeting(c.name)
        + body
  ...
  nps_survey: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Η γνώμη σου') + h('Μια ερώτηση, ειλικρινά') + greeting(c.name)
        + p('Σε μια κλίμακα από το 0 ως το 10, πόσο πιθανό είναι να πρότεινες το Property OS σε κάποιον με ακίνητα;')
  ...
  feedback_lottery: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Η γνώμη σου') + h('Η γνώμη σου μετράει, πραγματικά') + greeting(c.name)
        + p('Είδαμε ότι δεν έχεις μοιραστεί ακόμη τη γνώμη σου. Αφιέρωσε μόλις ένα λεπτό να μας πεις τις σκέψεις, τις ιδέες και την άποψή σου για το Property OS.')
        + bullets([
  ...
  best_practice_tip: (c) => {
    ...
    return { subject: c.headline ? esc(c.headline) : 'Η συμβουλή της εβδομάδας', html: emailShell({
      ...
      bodyHtml: eyebrow('Συμβουλή') + h(c.headline ? esc(c.headline) : 'Μια συμβουλή που πιάνει τόπο') + greeting(c.name)
        + tip
        + button('Εφάρμοσέ το στο Property OS', dash(c))
...
export const CONVERSION: Record<string, CopyFn> = {
  ...
  social_proof: (c) => ({
    subject: 'Γιατί οι επαγγελματίες επιλέγουν το Property OS',
    html: emailShell({
      ...
      bodyHtml: eyebrow('Γιατί Property OS') + h('Φτιαγμένο για την ελληνική πραγματικότητα') + greeting(c.name)
        + p('Ιδιοκτήτες και διαχειριστές εμπιστεύονται το Property OS για μια δουλειά που πριν έτρωγε ώρες: ενοίκια, έξοδα, φόρους και υπενθυμίσεις σε ένα μέρος.')
        + bullets([
  ...
  str_stay_tax: (c) => {
    ...
    return { subject: 'Απόδοση τέλους ανθεκτικότητας', html: emailShell({
      ...
      bodyHtml: eyebrow('Συμμόρφωση') + h('Ώρα για το τέλος ανθεκτικότητας') + greeting(c.name)
        + p(`Το <b>τέλος ανθεκτικότητας στην κλιματική κρίση</b> για τις βραχυχρόνιες μισθώσεις${per} εισπράττεται από τον επισκέπτη και αποδίδεται στην εφορία.${amt} Το Property OS υπολογίζει ενδεικτικά τι αναλογεί ανά κράτηση, ώστε να έχεις μια καθαρή εικόνα.`)
  ...
  tenant_welcome: (c) => {
    ...
    return { subject: 'Καλωσόρισες στο νέο σου σπίτι', html: emailShell({
      ...
      bodyHtml: eyebrow('Καλωσόρισμα') + h('Μια ξεκάθαρη αρχή') + greeting(c.name)
        + p(`Καλωσόρισες${ref}. Η διαχείριση του ακινήτου γίνεται μέσω Property OS, ώστε οι πληρωμές, οι αποδείξεις και η επικοινωνία να είναι απλές και ξεκάθαρες για όλους.${rent}`)
...
// Στιγμές όπου το Property OS βάζει χρήματα στην τσέπη του χρήστη ή του λύνει
...
export const VALUE: Record<string, CopyFn> = {
  ...
  energy_savings: (c) => {
    ...
    return { subject: 'Πλήρωσε λιγότερο για το ρεύμα', html: emailShell({
      ...
      bodyHtml: eyebrow('Εξοικονόμηση') + h('Το ρεύμα σου, φθηνότερα') + greeting(c.name)
        + p(`Το Property OS συγκρίνει τα προγράμματα ρεύματος και αερίου${ref} και σου δείχνει ποιο συμφέρει για τη δική σου κατανάλωση. Χωρίς τηλεφωνήματα, χωρίς ψιλά γράμματα.`)
        + save
        + button('Δες τη σύγκριση', dash(c))
  ...
  insurance_enfia: (c) => {
    ...
    return { subject: 'Ασφάλισε το ακίνητο, δες αν κερδίζεις και στον ΕΝΦΙΑ', html: emailShell({
      ...
      bodyHtml: eyebrow('Αξία') + h('Δύο οφέλη με μία κίνηση') + greeting(c.name)
        + p(`Ένα ασφαλισμένο ακίνητο δεν σε προστατεύει μόνο από ζημιές. Για ασφαλισμένα ακίνητα ενδέχεται, υπό προϋποθέσεις, να προβλέπεται και μείωση στον ΕΝΦΙΑ. Το Property OS σε βοηθά να δεις τι ταιριάζει${ref} και τι μπορεί να σημαίνει για σένα.`)
  ...
  loan_costs: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Δάνεια') + h('Πριν υπογράψεις, δες τα νούμερα') + greeting(c.name)
        + p('Ένα στεγαστικό δεν είναι μόνο η δόση. Το Property OS σου δείχνει το πλήρες κόστος: επιτόκιο, έξοδα φακέλου, συμβολαιογραφικά και μεταγραφή, ώστε να δεις ένα ρεαλιστικό, ενδεικτικό κόστος.')
        + bullets([
            'Σύγκριση δανείων και δόσεων, με τα δικά σου νούμερα.',
  ...
  rate_alert: (c) => {
    const line = c.summaryText
      ? p(esc(c.summaryText))
      : p('Το Property OS κρατά ενημερωμένες τις τραπεζικές τιμές και υπολογίζει τη δόση με τα δικά σου νούμερα. Έτσι, όποτε σκέφτεσαι δάνειο ή αναχρηματοδότηση, η σύγκριση είναι ήδη έτοιμη.');
  ...
  document_pack: (c) => {
    ...
    return { subject: 'Ο φάκελος του ακινήτου σου, με ένα κλικ', html: emailShell({
      ...
      bodyHtml: eyebrow('Έγγραφα') + h('Όλη η εικόνα, σε ένα αρχείο') + greeting(c.name)
        + p(`Χρειάζεσαι να δείξεις την εικόνα${ref} σε κάποιον; Το Property OS βγάζει έναν καθαρό, επίσημο φάκελο με έσοδα, έξοδα, απόδοση και φορολογικά, έτοιμο να τον προσκομίσεις.`)
        + bullets([
            'Για τον λογιστή ή τον νομικό σου.',
            'Για την τράπεζα, σε αίτημα δανείου.',
            'Για έναν ενδιαφερόμενο αγοραστή ή επενδυτή.',
          ])
        + p('Και κάτι που θα εκτιμήσουν: κάθε έγγραφο φέρει κωδικό QR. Όποιος το λάβει τον σκανάρει και επιβεβαιώνει ότι εκδόθηκε γνήσια από το Property OS, χωρίς να εκτίθενται ποσά.')
    ...
    return { subject: 'Τρεις κινήσεις για μεγαλύτερη απόδοση', html: emailShell({
      ...
      bodyHtml: eyebrow('Απόδοση') + h('Βγάλε περισσότερα από το ακίνητό σου') + greeting(c.name)
        ...
        + bullets([
            ...
            'Μια ενεργειακή αναβάθμιση (ΠΕΑ, θέρμανση, κουφώματα) ρίχνει τα πάγια και ανεβάζει τη ζήτηση.',
            'Στη βραχυχρόνια, κλείσε τα κενά διαστήματα με τιμολόγηση που ακολουθεί τη ζήτηση και τη σεζόν.',
          ])
        + p('Το Property OS σου δείχνει την τρέχουσα απόδοση ανά ακίνητο και πού υπάρχει περιθώριο, με τα δικά σου νούμερα.')
  ...
  loan_first_scenario: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Δάνεια') + h('Το πρώτο σου σενάριο δανείου') + greeting(c.name)
        + p('Μόλις υπολόγισες ένα σενάριο δανείου στο Property OS, καλή κίνηση. Πριν προχωρήσεις με μια τράπεζα, να τα σημεία που κάνουν τη μεγαλύτερη διαφορά στο τελικό κόστος:')
        + bullets([
            ...
            'Κοίτα το <b>ΣΕΠΠΕ</b>, το συνολικό ετήσιο πραγματικό κόστος, όχι μόνο το επιτόκιο: περιλαμβάνει έξοδα φακέλου, εκτίμησης και υποχρεωτικές ασφάλειες.',
            'Ρώτα για τους όρους πρόωρης εξόφλησης και για το τι ασφάλειες απαιτεί η κάθε τράπεζα.',
            'Αν πληροίς τις προϋποθέσεις, έλεγξε τα κρατικά προγράμματα στέγασης, όταν είναι ενεργά.',
          ])
        + p('Στο Property OS συγκρίνεις τράπεζες και δόσεις με τα δικά σου νούμερα και βλέπεις το πλήρες, ενδεικτικό κόστος πριν δεσμευτείς.')
...
export const NEWS: Record<string, CopyFn> = {
  ...
  news_rate_move: (c) => {
    ...
    return { subject: c.headline ? esc(c.headline) : 'Τα επιτόκια κινήθηκαν. Δες τι σημαίνει για σένα', html: emailShell({
      ...
      bodyHtml: eyebrow('Επικαιρότητα') + h('Τα επιτόκια κινήθηκαν') + greeting(c.name)
        + ctx
        + p('Στο Property OS συγκρίνεις <b>τραπεζικές τιμές</b>, υπολογίζεις τη νέα δόση με τα δικά σου νούμερα και βλέπεις αν συμφέρει μια <b>αναχρηματοδότηση</b>.')
  ...
  news_insurance_risk: (c) => {
    ...
    return { subject: c.headline ? esc(c.headline) : 'Προστάτεψε το ακίνητό σου, με το σωστό ασφαλιστήριο', html: emailShell({
      ...
      bodyHtml: eyebrow('Επικαιρότητα') + h('Η σιγουριά ξεκινά από την προστασία') + greeting(c.name)
        + ctx
        + p('Το Property OS σε βοηθά να δεις <b>ποιο ασφαλιστήριο κατοικίας ταιριάζει</b> στο ακίνητό σου και να συγκρίνεις, ώστε να μην πληρώνεις παραπάνω.')
        + bullets([
  ...
  news_tax_change: (c) => {
    ...
    return { subject: c.headline ? esc(c.headline) : 'Νέα ρύθμιση για τα ακίνητα. Τι αλλάζει για σένα', html: emailShell({
      ...
      bodyHtml: eyebrow('Επικαιρότητα') + h('Κάτι άλλαξε. Ας το δούμε καθαρά') + greeting(c.name)
        + ctx
        + p('Το Property OS κρατά τα δεδομένα σου ενημερωμένα και έτοιμα, ώστε η όποια αλλαγή να σε βρίσκει σε τάξη, όχι στην τελευταία στιγμή.')
  ...
  news_market: (c) => {
    const ctx = c.summaryText ? p(esc(c.summaryText)) : p('Η αγορά ακινήτων κινείται. Μια ματιά στα δικά σου νούμερα σε κρατά ένα βήμα μπροστά.')
  ...
  news_utility_prices: (c) => {
    ...
    return { subject: c.headline ? esc(c.headline) : 'Άλλαξαν οι τιμές σε ρεύμα και internet. Μήπως αξίζει καλύτερο πρόγραμμα;', html: emailShell({
      ...
      bodyHtml: eyebrow('Επικαιρότητα') + h('Οι τιμές κινήθηκαν. Δες αν πληρώνεις παραπάνω') + greeting(c.name)
        + ctx
        + p('Στο Property OS συγκρίνεις προγράμματα <b>ρεύματος</b>, <b>φυσικού αερίου</b> και <b>internet</b> για το ακίνητό σου και βλέπεις ποιο συμφέρει για τη δική σου κατανάλωση, χωρίς τηλεφωνήματα και χωρίς ψιλά γράμματα.')
