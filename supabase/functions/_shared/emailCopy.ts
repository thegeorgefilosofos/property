...
export const LIFECYCLE: Record<string, CopyFn> = {
  ...
  // 46α. Επαναφορά κωδικού (transactional, ασφάλεια)
  password_reset: (c) => ({
    subject: 'Επανάφερε τον κωδικό σου',
    html: emailShell({
      preheader: 'Ένα κλικ για νέο κωδικό.',
      bodyHtml: eyebrow('Ασφάλεια') + h('Ας ορίσουμε νέο κωδικό') + greeting(c.name)
        + p('Λάβαμε αίτημα για επαναφορά του κωδικού πρόσβασης στον λογαριασμό σου. Πάτησε το κουμπί παρακάτω για να ορίσεις έναν νέο κωδικό.')
        + p('Αν δεν ζήτησες εσύ αυτή την αλλαγή, αγνόησε το μήνυμα. Ο κωδικός σου παραμένει ίδιος και ο λογαριασμός σου ασφαλής.')
        + button('Όρισε νέο κωδικό', dash(c))
        + note('Ο σύνδεσμος λήγει σε 30 λεπτά και χρησιμοποιείται μία φορά. Δεν ζητάμε ποτέ τον κωδικό σου μέσω email.'),
    }),
  }),
  ...
  deposit_reminder: (c) => {
    ...
    return { subject: 'Η εγγύηση θέλει τακτοποίηση', html: emailShell({
      ...
      bodyHtml: eyebrow('Εγγύηση') + h('Ας κλείσουμε καθαρά την εγγύηση') + greeting(c.name)
        ...
        + p('Κατάγραψε την κίνηση στο Property OS, ώστε τα βιβλία σου να μένουν καθαρά και το κλείσιμο της μίσθωσης ξεκάθαρο για κάθε πλευρά. Η απόδειξη μένει στο ιστορικό του ακινήτου, έτοιμη όποτε χρειαστεί.')
        + bullets([
  ...
  milestone_reached: (c) => {
    ...
    return { subject: 'Ένα ορόσημο που αξίζει αναγνώριση', html: emailShell({
      ...
      bodyHtml: eyebrow('Ορόσημο') + h('Έφτασες κάπου που μετράει') + greeting(c.name)
        + p(`Μόλις πέρασες ${what} μέσα από το Property OS. Πίσω από αυτό το δεδομένο κρύβονται ώρες που κέρδισες και εκκρεμότητες που δεν σε απασχόλησαν.`)
  ...
  takk_seasonal_rate_switch: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Συμμόρφωση · Βραχυχρόνια') + h('Νέα εποχική χρέωση από σήμερα') + greeting(c.name)
        ...
        + p('Το τέλος εισπράττεται από τον επισκέπτη και αποδίδεται στην εφορία. Το Property OS υπολογίζει ενδεικτικά τι αναλογεί ανά κράτηση με τη νέα τιμή.')
...
export const VALUE: Record<string, CopyFn> = {
  ...
  energy_savings: (c) => {
    ...
    return { subject: 'Πλήρωσε λιγότερο για το ρεύμα', html: emailShell({
      ...
      bodyHtml: eyebrow('Εξοικονόμηση') + h('Το ρεύμα σου, φθηνότερα') + greeting(c.name)
        + p(`Το Property OS συγκρίνει τα προγράμματα ρεύματος, φυσικού αερίου και internet${ref} και σου δείχνει ποιο συμφέρει για τη δική σου κατανάλωση. Χωρίς τηλεφωνήματα, χωρίς ψιλά γράμματα.`)
        + save
        + button('Δες τη σύγκριση', dash(c))
  ...
  rate_alert: (c) => {
    const line = c.summaryText
      ? p(esc(c.summaryText))
      : p('Το Property OS κρατά ενημερωμένες τις τραπεζικές τιμές και υπολογίζει τη δόση με τα δικά σου δεδομένα. Έτσι, όποτε σκέφτεσαι δάνειο ή αναχρηματοδότηση, η σύγκριση είναι ήδη έτοιμη.');
  ...
  yield_boost: (c) => {
    ...
    return { subject: 'Τρεις κινήσεις για μεγαλύτερη απόδοση', html: emailShell({
      ...
      bodyHtml: eyebrow('Απόδοση') + h('Βγάλε περισσότερα από το ακίνητό σου') + greeting(c.name)
        ...
        + p('Το Property OS σου δείχνει την τρέχουσα απόδοση ανά ακίνητο και πού υπάρχει περιθώριο, με τα δικά σου δεδομένα.')
  ...
  loan_first_scenario: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Δάνεια') + h('Το πρώτο σου σενάριο δανείου') + greeting(c.name)
        ...
        + p('Στο Property OS συγκρίνεις τράπεζες και δόσεις με τα δικά σου δεδομένα και βλέπεις το πλήρες, ενδεικτικό κόστος πριν δεσμευτείς.')
  ...
  news_rate_move: (c) => {
    ...
    return { subject: c.headline ? esc(c.headline) : 'Τα επιτόκια κινήθηκαν. Δες τι σημαίνει για σένα', html: emailShell({
      ...
      bodyHtml: eyebrow('Επικαιρότητα') + h('Τα επιτόκια κινήθηκαν') + greeting(c.name)
        + ctx
        + p('Στο Property OS συγκρίνεις <b>τραπεζικές τιμές</b>, υπολογίζεις τη νέα δόση με τα δικά σου δεδομένα και βλέπεις αν συμφέρει μια <b>αναχρηματοδότηση</b>.')
