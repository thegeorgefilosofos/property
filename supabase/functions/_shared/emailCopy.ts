  ...
  tax_e2: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Φορολογικό ημερολόγιο') + h('Ώρα για το Ε2') + greeting(c.name)
        + p(`Η προθεσμία για τη δήλωση εισοδήματος${c.deadlineDate ? ` (έως ${esc(c.deadlineDate)})` : ''} πλησιάζει. Το Property OS συγκεντρώνει τα ενοίκιά σου ανά ακίνητο και ετοιμάζει το Ε2 σου προσυμπληρωμένο, με τα ποσά στη σωστή στήλη, έτοιμο για τον λογιστή σου ή για το myAADE.`)
  ...
  feedback_lottery: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Η γνώμη σου') + h('Η γνώμη σου μετράει, πραγματικά') + greeting(c.name)
        + p('Πρόσεξα ότι δεν έχεις μοιραστεί ακόμη τη γνώμη σου. Αφιέρωσε μόλις ένα λεπτό να μας πεις τις σκέψεις, τις ιδέες και την άποψή σου για το Property OS.')
        + bullets([
        ...
        + button('Πες μας τη γνώμη σου', dash(c))
        + note('Θα σου πάρει ένα λεπτό. Διαβάζουμε κάθε απάντηση, μία μία.'),
    }),
  }),
...
export const VALUE: Record<string, CopyFn> = {
  ...
  insurance_enfia: (c) => {
    ...
    return { subject: 'Ασφάλισε το ακίνητο, κέρδισε και στον ΕΝΦΙΑ', html: emailShell({
      ...
      bodyHtml: eyebrow('Αξία') + h('Δύο οφέλη με μία κίνηση') + greeting(c.name)
        + p(`Ένα ασφαλισμένο ακίνητο δεν σε προστατεύει μόνο από ζημιές. Για τα ασφαλισμένα ακίνητα προβλέπεται και μείωση στον ΕΝΦΙΑ. Το Property OS σε βοηθά να δεις τι ταιριάζει${ref} και τι μπορείς να κερδίσεις.`)
  ...
  loan_costs: (c) => ({
    ...
    html: emailShell({
      ...
      bodyHtml: eyebrow('Δάνεια') + h('Πριν υπογράψεις, δες τα νούμερα') + greeting(c.name)
        + p('Ένα στεγαστικό δεν είναι μόνο η δόση. Το Property OS σου δείχνει το πλήρες κόστος: επιτόκιο, έξοδα φακέλου, συμβολαιογραφικά και μεταγραφή, ώστε να ξέρεις τι πραγματικά πληρώνεις.')
        + bullets([
            'Σύγκριση δανείων και δόσεων, με τα δικά σου νούμερα.',
            'Έλεγχος επιλεξιμότητας για προγράμματα όπως το «Σπίτι μου».',
