...
export const VALUE: Record<string, CopyFn> = {
  ...
  // 99. Ειδοποίηση μεταβολής επιτοκίων
  rate_alert: (c) => {
    const line = c.summaryText
      ? p(esc(c.summaryText))
      : p('Τα επιτόκια μεταβλήθηκαν. Αν έχεις δάνειο με κυμαινόμενο επιτόκιο ή σκέφτεσαι να δανειστείς, αυτή η αλλαγή σε αφορά.');
    return { subject: 'Τα επιτόκια άλλαξαν. Δες τι σημαίνει για σένα', html: emailShell({
      preheader: 'Μια μεταβολή που ίσως αλλάζει τη δόση σου.', unsubUrl: c.unsubUrl,
      bodyHtml: eyebrow('Επιτόκια') + h('Μια κίνηση που σε αφορά') + greeting(c.name)
        + line
        + p('Το Property OS υπολογίζει πώς η μεταβολή επηρεάζει τη δόση σου και αν συμφέρει μια αναχρηματοδότηση.')
        + button('Δες την επίδραση', dash(c))
        + note('Ενημερωτικά, με βάση τα δημόσια στοιχεία της αγοράς.'),
    }) };
  },
