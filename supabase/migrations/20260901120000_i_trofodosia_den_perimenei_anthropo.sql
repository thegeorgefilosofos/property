...
--
-- ΔΙΟΡΘΩΣΗ ΤΟΥ ΙΔΙΟΥ ΜΟΥ ΙΣΧΥΡΙΣΜΟΥ, ΓΡΑΜΜΕΝΗ ΕΔΩ ΓΙΑ ΝΑ ΜΗ ΜΕΙΝΕΙ ΨΕΜΑ ΣΤΟ
-- ΑΡΧΕΙΟ. Η παρακάτω παράγραφος (α) λέει ότι η εργασία «δεν θα έτρεχε ποτέ»
-- επειδή έλειπε το μυστικό του vault. Οταν κοιτάχτηκε η παραγωγή, το
-- `market_data_fn_url` ΥΠΗΡΧΕ και η εργασία των 08:00 έτρεξε κανονικά. Η
-- πραγματική αιτία που η τροφοδοσία δεν έδινε τίποτα ήταν τα παλιά κλειδιά
-- σειρών («RT0»), που διορθώθηκαν νωρίτερα την ίδια μέρα.
--
-- Η αλλαγή παραμένει σωστή — δύο εργασίες είχαν άλλον τρόπο από τις άλλες
-- πέντε και ένα ξεχασμένο μυστικό θα τις έσβηνε σιωπηλά — αλλά ήταν ΕΥΘΡΑΥΣΤΟ
-- ΣΗΜΕΙΟ, όχι η αιτία. Το ίδιο και το (β): το μυστικό υπήρχε ήδη· η
-- μετανάστευση απλώς εγγυάται ότι θα υπάρχει και σε κάθε νέο περιβάλλον.
--
