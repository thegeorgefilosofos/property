-- ═══════════════════════════════════════════════════════════════════════════
-- Lifecycle enqueue engine — activates the (already excellent) email catalog.
--
-- Until now the 111-email catalog + the tested cadence planner sat idle:
-- enqueue_email() had zero callers. This function is the "brain" that reads real
-- customer data once a day and enqueues the RIGHT catalog email for each user's
-- situation, then hands off to the existing pipeline:
--
--   lifecycle_enqueue()  →  email_outbox (send_window NULL)
--     →  schedule-email-outbox (applies the anti-spam cadence policy)
--     →  drain_email_outbox   (sends, but ONLY once emails_live='1')
--
-- Every insert carries a dedup_key, so running daily is idempotent — a user is
-- never enqueued the same message twice for the same window. Categories:
-- 'operational' = obligations the owner must see (expiry reminders); 'marketing'
-- = lifecycle/seasonal/growth (subject to the 3-day marketing cap + the master
-- emails_live switch). Nothing here can reach a real inbox until the owner flips
-- emails_live on after verifying the sending domain (see the safety migration).
--
-- SECURITY DEFINER (owner=postgres) so it may read auth.users for confirmed
-- addresses. Only confirmed, non-deleted users are ever targeted.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ── Schedule it: daily 07:05 UTC, guarded so it is safe without pg_cron. ─────
do $$
begin
  if not exists (select 1 from pg_extension where extname = 'pg_cron') then
    return;
  end if;
  if exists (select 1 from cron.job where jobname = 'lifecycle-enqueue-daily') then
    perform cron.unschedule('lifecycle-enqueue-daily');
  end if;
  perform cron.schedule('lifecycle-enqueue-daily', '5 7 * * *', $cron$ select public.lifecycle_enqueue(); $cron$);
end $$;
