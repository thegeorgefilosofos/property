...
const SUBPROCESSORS: { name: string; what: string; where: string }[] = [
  { name: 'Supabase', what: 'Η βάση δεδομένων, η σύνδεση και τα αρχεία σου — η κύρια πλατφόρμα', where: 'ΕΕ · Φρανκφούρτη' },
  { name: 'Vercel', what: 'Φιλοξενία και παράδοση της εφαρμογής', where: 'Παγκόσμιο δίκτυο' },
  { name: 'Resend', what: 'Αποστολή email (υπενθυμίσεις, ειδοποιήσεις)', where: 'ΗΠΑ · με τυποποιημένες ρήτρες' },
  { name: 'Anthropic', what: 'Ο βοηθός: απαντά στις ερωτήσεις σου', where: 'ΗΠΑ · με τυποποιημένες ρήτρες' },
  { name: 'GitHub', what: 'Κώδικας και κρυπτογραφημένα αντίγραφα ασφαλείας', where: 'ΗΠΑ · με τυποποιημένες ρήτρες' },
];
    ...
    <div style={{ background: 'var(--bg-base)', color: 'var(--text-primary)', minHeight: '100vh', fontFamily: T.font.sans }}>
      ...
      <main style={{ maxWidth: 780, margin: '0 auto', padding: 'clamp(32px,6vw,64px) clamp(20px,5vw,40px)' }}>
        ...
        <section style={SECTION}>
          ...
          <p style={P}>
            Η βάση δεδομένων, οι κωδικοί σύνδεσης και τα αρχεία που ανεβάζεις βρίσκονται σε διακομιστές
            <strong style={{ color: 'var(--text-primary)' }}> εντός Ευρωπαϊκής Ένωσης, στη Φρανκφούρτη</strong>.
            Δεν αντιγράφονται σε διακομιστές εκτός ΕΕ. Η μεταφορά γίνεται πάντα κρυπτογραφημένα (TLS)
            και τα δεδομένα φυλάσσονται κρυπτογραφημένα στον δίσκο.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 16 }}>
            ...
            <Point title="Καθημερινό αντίγραφο ασφαλείας">
              Κρατάμε καθημερινό αντίγραφο, κρυπτογραφημένο με AES-256 πριν αποθηκευτεί. Χωρίς το κλειδί
              κρυπτογράφησης το αντίγραφο δεν ανοίγει — ούτε από εμάς.
            </Point>
            ...
            <Point title="Ιστορικό ενεργειών">
              Κάθε σημαντική ενέργεια στον λογαριασμό σου καταγράφεται και μπορείς να τη δεις ο ίδιος,
              στον Λογαριασμό → Ιστορικό.
            </Point>
          </div>
        </section>
          ...
          <div style={{ marginTop: 14, border: '1px solid var(--border-subtle)', borderRadius: 12, overflow: 'hidden' }}>
            {SUBPROCESSORS.map((s, i) => (
              <div key={s.name} style={{ display: 'grid', gridTemplateColumns: 'minmax(90px, 0.7fr) minmax(0, 1.6fr) minmax(110px, 0.9fr)', gap: 12, padding: '12px 14px', borderTop: i === 0 ? 'none' : '1px solid var(--border-subtle)', background: 'var(--bg-surface)' }}>
                <span style={{ fontSize: 13, fontWeight: 650, color: 'var(--text-primary)' }}>{s.name}</span>
