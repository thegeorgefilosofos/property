    ...
    <div className="lp-root" style={{ color: TEXT, minHeight: '100vh', fontFamily: T.font.sans, overflowX: 'clip', position: 'relative' }}>
      ...
      <style>{`
        ...
        .lp-why p {
          ...
          text-align: justify; text-align-last: left; hyphens: auto; text-wrap: pretty;
        }
        @media (max-width: 860px) { .lp-why { grid-template-columns: minmax(0, 1fr); gap: 16px; } }
        /* Κάτω από τις 560 η στήλη είναι πολύ στενή για στοίχιση: ούτε ο
           συλλαβισμός σώζει γραμμή τριάντα χαρακτήρων. */
        @media (max-width: 560px) { .lp-why p { text-align: left; hyphens: none; } }
        ...
        <div className="lp-split" style={{ background: PANEL, border: `1px solid ${LINE}`, borderRadius: 14, padding: 'clamp(24px, 3vw, 38px)', display: 'grid', gridTemplateColumns: 'minmax(0, 0.82fr) minmax(0, 1.18fr)', gap: 'clamp(24px, 3vw, 44px)', alignItems: 'start' }}>
          <div>
            ...
            <p style={{ fontSize: 15, color: MUTED, lineHeight: 1.6, margin: 0 }}>
              Λογαριασμοί, συμβόλαια και στοιχεία του ακινήτου ή των πελατών σου: αποτελούν από τα πιο ευαίσθητα δεδομένα που υπάρχουν, γι’ αυτό και τα διατηρούμε ασφαλή.
            </p>
          </div>
      ...
      <section className="lp-reveal" style={{ ...wrap, position: 'relative', zIndex: 1, paddingBottom: GAP_ACT }}>
        ...
        {/* ═══ ΔΥΟ ΜΠΛΟΚ, ΟΧΙ ΤΕΣΣΕΡΑ ══════════════════════════════════════════
            ΤΙ ΗΤΑΝ ΧΑΛΙΑ. Κάθε στήλη είχε ΔΥΟ παραγράφους διαφορετικού μήκους.
            Το άθροισμα των χαρακτήρων ήταν ζυγισμένο (309 έναντι 318) και παρ'
            όλα αυτά η αριστερή έβγαζε έξι γραμμές και η δεξιά πέντε: το ζύγισμα
            ισχύει για ΣΥΝΕΧΟΜΕΝΟ κείμενο, ενώ κάθε παράγραφος στρογγυλοποιεί
            προς τα πάνω στην τελευταία της γραμμή. Δύο παράγραφοι ανά στήλη
            σημαίνουν δύο μισοάδειες γραμμές, και οι δύο πλευρές τελείωναν σε
            διαφορετικό ύψος.

            ΤΩΡΑ. Μία παράγραφος ανά στήλη, ίδιου μήκους σε χαρακτήρες, οπότε οι
            δύο στήλες κλείνουν στην ίδια γραμμή.

            ΚΑΙ Ο ΣΥΛΛΑΒΙΣΜΟΣ. Η στοίχιση «πέρα πέρα» σε στήλη μισού πλάτους, με
            ελληνικές λέξεις δώδεκα γραμμάτων, ανοίγει ποτάμια λευκού ανάμεσα
            στις λέξεις — αυτό ήταν το «τσαπατσουλικο» της οθόνης, όχι η
            στοίχιση καθαυτή. Με `hyphens: auto` και τη σελίδα δηλωμένη ως
            ελληνική (`lang="el"` στο app/layout.tsx), ο περιηγητής συλλαβίζει
            και τα κενά μένουν κανονικά. Το `text-align-last: left` αφήνει την
            τελευταία γραμμή να τελειώσει φυσικά, όπως ορίζει η τυπογραφία. */}
        <div className="lp-why">
          <p>
            Αν έχεις ακίνητα στην Ελλάδα, το ξέρεις. Το φορολογικό πλαίσιο αλλάζει κάθε χρόνο, οι
            υποχρεώσεις πληθαίνουν, οι προθεσμίες δεν περιμένουν. Ε2, Δήλωση Μίσθωσης, ΕΝΦΙΑ,
            τεκμαρτή έκπτωση, κοινόχρηστα, αναπροσαρμογή μισθώματος, δηλώσεις βραχυχρόνιας:
            καθένα με τη δική του προθεσμία, και η ευθύνη πάντα πάνω σου.
          </p>
          <p>
            Γι’ αυτό σχεδιάσαμε το Property OS. Τα πάντα για το ακίνητο σε ένα σημείο, φορολογικά,
            νομικά, οικονομικά, πλήρως ενημερωμένα. Ο υπολογισμός βγαίνει πριν τον ζητήσεις. Η
            προθεσμία εμφανίζεται πριν τη θυμηθείς. Το παραστατικό καταχωρείται με μια φωτογραφία.
