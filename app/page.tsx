...
const FAQ = [
  { q: 'Πόσο κοστίζει;', a: `Τέσσερα πακέτα: από ${fe(PLANS.solo.priceMonthly)} τον μήνα για ένα ακίνητο με όλα τα φορολογικά, ως ${fe(PLANS.office.priceMonthly)} τον μήνα για απεριόριστα ακίνητα και χρήστες. Με ετήσια πληρώνεις λιγότερους μήνες από όσους παίρνεις.` },
  ...
  { q: 'Τι γίνεται όταν λήξει η δοκιμή;', a: `${billingWords().afterTrial} Το πακέτο που διάλεξες στην εγγραφή είναι εκείνο που θα ισχύσει.` },
  ...
  { q: 'Δουλεύει στο κινητό;', a: 'Παντού: η εφαρμογή προσαρμόζεται σε κινητό, tablet και υπολογιστή, και η σάρωση δουλεύει καλύτερα με την κάμερα του κινητού. Εγκαθίσταται και στην αρχική οθόνη σαν εφαρμογή, χωρίς κατάστημα. Ξεχωριστή εφαρμογή για iOS και Android δεν υπάρχει ακόμη και δεν έχει ημερομηνία· στον Λογαριασμό υπάρχει λίστα αναμονής και ειδοποιούμε όσους είναι μέσα.' },
    ...
    <div className="lp-root" style={{ color: TEXT, minHeight: '100vh', fontFamily: T.font.sans, overflowX: 'clip', position: 'relative' }}>
      ...
      <style>{`
        ...
        /* ═══ Η ΣΤΟΙΧΙΣΗ «ΠΕΡΑ ΠΕΡΑ» ΕΦΥΓΕ, ΚΑΙ ΕΙΝΑΙ Η ΣΩΣΤΗ ΑΠΟΦΑΣΗ ═══════════
           ...
           ολόκληρο το μέτρο της ενότητας, όσο και ο τίτλος από πάνω.
        ...
        .lp-why { max-width: 1044px; display: flex; flex-direction: column; gap: clamp(14px, 1.6vw, 18px); }
        .lp-why p {
        ...
        <div className="lp-split" style={{ background: PANEL, border: `1px solid ${LINE}`, borderRadius: T.radius.card, padding: 'clamp(24px, 3vw, 38px)', display: 'grid', gridTemplateColumns: 'minmax(0, 0.9fr) minmax(0, 1.1fr)', gap: 'clamp(24px, 3vw, 44px)', alignItems: 'start' }}>
          <div>
            ...
            <p style={{ fontSize: 15, color: MUTED, lineHeight: 1.6, margin: 0 }}>
              {/* ΤΟ «ΤΩΝ» ΔΕΝ ΜΕΝΕΙ ΜΟΝΟ ΤΟΥ ΣΤΟ ΤΕΛΟΣ ΤΗΣ ΠΡΩΤΗΣ ΓΡΑΜΜΗΣ. Ενα
                  άρθρο κρεμασμένο στην άκρη σπάει το ζευγάρι που διαβάζεται
                  μαζί: το μάτι το προσπερνά και ξαναρχίζει στη δεύτερη γραμμή
                  από το «πελατών». Το ακατάλυτο κενό κρατά «των πελατών»
                  ενωμένο, οπότε κατεβαίνουν και τα δύο. */}
              Λογαριασμοί, συμβόλαια και στοιχεία του ακινήτου ή των{'\u00A0'}πελατών σου: αποτελούν από τα πιο ευαίσθητα δεδομένα, γι’ αυτό και τα διατηρούμε ασφαλή.
            </p>
          </div>
          {/* ── ΜΙΑ ΣΤΗΛΗ ΓΙΑ ΤΙΣ ΤΕΣΣΕΡΙΣ ΕΓΓΥΗΣΕΙΣ, ΟΧΙ ΔΥΟ ΕΠΙ ΔΥΟ ──────────
              Οι τίτλοι («Κρυπτογραφημένη σύνδεση», «Καμία εκπαίδευση
              μοντέλων») χωρούσαν σε μία γραμμή με διαφορά δύο εικονοστοιχείων:
              ολόκληρη η ισορροπία της κάρτας κρεμόταν από το πλάτος του
              μακρύτερου τίτλου, και κάθε λέξη που θα προστίθετο κάπου αλλού
              την έσπαγε. Σε μία στήλη κανένας τίτλος δεν αναδιπλώνεται ποτέ,
              και οι τέσσερις εγγυήσεις διαβάζονται ως λίστα — που είναι.
              ΤΟ ΤΙΜΗΜΑ, ΓΡΑΜΜΕΝΟ: η δεξιά στήλη γίνεται ψηλότερη από την
              αριστερή και μένει κενό κάτω από την εισαγωγή. Προτιμήθηκε από
              την εναλλακτική, που ήταν τίτλοι σπασμένοι στα δύο δίπλα σε
              τίτλους μιας γραμμής — ανομοιομορφία που τη βλέπει ο αναγνώστης,
              ενώ το κενό απλώς αερίζει. */}
          ...
          {/* Η ΑΠΟΚΑΛΥΨΗ ΓΙΑ ΤΟΥΣ ΠΑΡΟΧΟΥΣ ΕΚΤΟΣ ΕΕ ΔΕΝ ΧΑΝΕΤΑΙ, ΑΛΛΑΖΕΙ ΘΕΣΗ.
              ...
              ΚΑΙ ΠΙΑΝΕΙ ΟΛΟ ΤΟ ΠΛΑΤΟΣ ΤΗΣ ΚΑΡΤΑΣ, ΟΧΙ ΤΗ ΔΕΞΙΑ ΣΤΗΛΗ. Μέσα στη
              στήλη των εγγυήσεων έσπαγε στα δύο, δηλαδή μια υποσημείωση δύο
              γραμμών κάτω από τέσσερις εγγυήσεις μιας. Είναι υποσημείωση
              ΟΛΟΚΛΗΡΗΣ της ενότητας, οπότε ξεκινά από τον ίδιο άξονα με τον
              τίτλο και τελειώνει στην ίδια άκρη με το πλέγμα. */}
      ...
      <section id="faq" className="lp-reveal" style={{ ...wrap, position: 'relative', zIndex: 1, paddingBottom: 0 }}>
        ...
        <details className="lp-faq lp-faq-more">
          <summary style={{ cursor: 'pointer', listStyle: 'none', padding: '18px 0', fontSize: 14, fontWeight: 600, color: ACCENT, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
