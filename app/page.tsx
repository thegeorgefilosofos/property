...
const FEATURES = [
  ...
  { t: 'Βοηθός που σκέφτεται στα ελληνικά', d: 'Έχει μπροστά του τα δικά σου νούμερα, όχι γενικότητες. Με φωνή, αν σε βολεύει.', i: 'M21 12a8 8 0 0 1-8 8H8l-5 3 1.4-4.2A8 8 0 1 1 21 12M8.5 12h.01M12 12h.01M15.5 12h.01' },
  { t: 'Σύγκριση παρόχων ρεύματος και ασφάλισης', d: 'Τιμολόγια 11 παρόχων ρεύματος και 16 ασφαλιστικών, επαληθευμένα.', i: 'M3 12h4l3 8 4-16 3 8h4' },
  { t: 'Δάνειο χωρίς εκπλήξεις', d: 'Δόσεις, επιτόκια και έξοδα μεταβίβασης — και αν δικαιούσαι το «Σπίτι μου ΙΙ».', i: 'M3 21h18M5 21V7l8-4v18M19 21V11l-6-4' },
  { t: 'Αποδόσεις και σενάρια μίσθωσης', d: 'Καθαρή απόδοση ανά ακίνητο, και βραχυχρόνια δίπλα σε μακροχρόνια.', i: 'M12 2v20M17 7H9.5a2.5 2.5 0 0 0 0 5h5a2.5 2.5 0 0 1 0 5H7' },
  { t: 'Φορολογία 2026', d: 'Υποχρεώσεις και προθεσμίες με την κλίμακα που ισχύει, ανά έτος.', i: 'M9 7h6M9 11h6M9 15h4M5 3h14v18l-3-2-2 2-2-2-2 2-3-2z' },
  { t: 'Έγγραφα και προθεσμίες', d: 'Ένα αρχείο για όλα, κι ένα ημερολόγιο που θυμάται αντί για σένα.', i: 'M4 4h6l2 2h8v12H4zM4 10h16' },
];
...
const FAQ = [
  // ΚΑΘΕ ΑΠΑΝΤΗΣΗ ΔΥΟ ΓΡΑΜΜΕΣ. Ήταν 300-600 χαρακτήρες η καθεμία, δηλαδή πέντε
  // ως έξι σειρές: κανείς δεν διαβάζει έξι σειρές για να μάθει αν δουλεύει στο
  // κινητό. Ό,τι επαναλαμβανόταν από τον τιμοκατάλογο, τις κάρτες κοινού ή τη
  // «Γιατί υπάρχει» βγήκε — λέγεται ήδη μία φορά, στη θέση του.
  { q: 'Πώς δουλεύει η σάρωση με φωτογραφία;', a: 'Φωτογραφίζεις το έγγραφο και ο βοηθός το καταχωρεί μόνος του στο σωστό σημείο. Βλέπεις τι κατάλαβε πριν αποθηκευτεί.' },
  { q: 'Ο βοηθός μιλάει ελληνικά;', a: 'Γράφει, καταλαβαίνει και —σε Chrome ή Safari— σου μιλάει. Απαντά με βάση τα δικά σου ακίνητα και προθεσμίες.' },
  { q: 'Σε ποιους απευθύνεται;', a: 'Από το ένα διαμέρισμα ως το χαρτοφυλάκιο μεσιτικού γραφείου, για κάθε τύπο ακινήτου.' },
  { q: 'Αντικαθιστά τον λογιστή μου;', a: 'Όχι. Σε πάει στον λογιστή σου προετοιμασμένο, με τα στοιχεία έτοιμα για εξαγωγή.' },
  { q: 'Πόσο κοστίζει;', a: `Το πρώτο ακίνητο μένει δωρεάν για πάντα. Με ${fe(PLANS.solo.priceMonthly, 2)} τον μήνα ξεκλειδώνεις τα φορολογικά για ένα ακίνητο, και κάθε επιπλέον ακίνητο κοστίζει ${EXTRA_PROPERTY_PRICE} €. Κάθε λογαριασμός ξεκινά με ${TRIAL_DAYS} ημέρες δοκιμή.` },
  { q: 'Είναι ασφαλή τα δεδομένα μου;', a: 'Κρυπτογραφημένη σύνδεση, απομόνωση ανά λογαριασμό σε επίπεδο βάσης, αρχεία στην ΕΕ. Δεν εκπαιδεύουμε μοντέλα με τα έγγραφά σου.' },
  { q: 'Τι γίνεται αν σταματήσω;', a: 'Τα εξάγεις όποτε θέλεις και διαγράφεις τον λογαριασμό με ένα κουμπί. Η οριστική διαγραφή ολοκληρώνεται σε 30 ημέρες.' },
  { q: 'Δουλεύει στο κινητό;', a: 'Παντού — και η σάρωση δουλεύει καλύτερα εκεί: καταχωρείς τον λογαριασμό πριν φτάσεις σπίτι.' },
  { q: 'Πόσο γρήγορα ξεκινάω;', a: 'Λιγότερο από ένα λεπτό, με Google ή email. Υπάρχουν και έτοιμα δεδομένα επίδειξης.' },
];
...
const AUDIENCE = [
  ...
  // καθένα — τρεις στήλες κειμένου δίπλα-δίπλα, που δεν σαρώνονται.
  {
    tag: 'Ιδιοκτήτης',
    t: 'Ένα ή δύο ακίνητα',
    d: 'Τι πληρώνεις, τι εισπράττεις, τι λήγει.',
...
const REFERRAL = [
  ...
  // Ό,τι είναι όρος και όχι υπόσχεση πάει στη γραμμή των ψιλών γραμμάτων.
  {
    ...
    items: ['Ο φίλος σου παίρνει δύο μήνες δώρο', 'Εσύ κερδίζεις δωρεάν μήνες στη συνδρομή σου'],
  },
  {
    ...
    items: ['5 συνδρομητές → 2 δωρεάν μήνες', '20% επαναλαμβανόμενη προμήθεια ως Συνεργάτης'],
  },
];
