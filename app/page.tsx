...
const FEATURES = [
  ...
  { t: 'Αποδόσεις και σενάρια μίσθωσης', d: 'Καθαρή απόδοση μετά τον φόρο, και η βραχυχρόνια δίπλα στη μακροχρόνια μίσθωση, με τα δικά σου δεδομένα.', i: 'M12 2v20M17 7H9.5a2.5 2.5 0 0 0 0 5h5a2.5 2.5 0 0 1 0 5H7' },
  ...
  return (
    <div className="lp-root" style={{ color: TEXT, minHeight: '100vh', fontFamily: T.font.sans, overflowX: 'clip', position: 'relative' }}>
      ...
      <style>{`
        ...
        /* ΤΟ ΚΑΤΑΣΤΙΧΟ ΤΩΝ ΤΕΣΣΑΡΩΝ. Υποκείμενο αριστερά σε σταθερή στήλη, ρήμα
           δεξιά: τα τέσσερα ρήματα στοιχίζονται κάθετα και η ενότητα σαρώνεται
           αντί να διαβάζεται. Η πρώτη στήλη είναι max-content ώστε το πλάτος της
           να το ορίζει η μεγαλύτερη φράση, όχι ένας αριθμός γραμμένος στο χέρι
           που θα έσπαγε με το πρώτο κείμενο που αλλάζει. */
        .lp-ledger { margin: 0; max-width: 760px; border-top: 1px solid var(--border-subtle); }
        .lp-ledger-row {
          display: grid; grid-template-columns: max-content minmax(0, 1fr);
          column-gap: clamp(16px, 3vw, 40px); align-items: baseline;
          padding: clamp(11px, 1.4vw, 15px) 0; border-bottom: 1px solid var(--border-subtle);
        }
        .lp-ledger dt { font-size: clamp(14px, 1.45vw, 16px); color: var(--text-tertiary); }
        .lp-ledger dd { margin: 0; font-size: clamp(15px, 1.55vw, 17px); font-weight: 600; color: var(--text-primary); letter-spacing: -0.01em; }
        /* Σε κινητό οι δύο στήλες γίνονται δύο σειρές: με πλάτος 360 η δεύτερη
           στήλη έμενε τέσσερις χαρακτήρες και το ρήμα έσπαγε σε τρεις γραμμές. */
        @media (max-width: 560px) {
          .lp-ledger-row { grid-template-columns: minmax(0, 1fr); row-gap: 2px; }
        }
      ...
      <section className="lp-reveal" style={{ ...wrap, position: 'relative', zIndex: 1, paddingBottom: GAP_ACT }}>
        ...
        {/* ═══ ΑΠΟ ΤΕΣΣΕΡΙΣ ΠΑΡΑΓΡΑΦΟΥΣ ΣΕ ΔΥΟ ΠΡΟΤΑΣΕΙΣ ΚΑΙ ΕΝΑ ΚΑΤΑΣΤΙΧΟ ══════
            ΤΙ ΔΕΝ ΔΟΥΛΕΥΕ. Δύο στήλες με δύο στοιχισμένες παραγράφους η καθεμία
            είναι διάταξη εφημερίδας: τέσσερα μπλοκ ίδιου βάρους, καμία ιεραρχία,
            και ο αναγνώστης δεν ξέρει ποιο να διαβάσει πρώτο. Η στοίχιση «πέρα
            πέρα» άνοιγε ποτάμια ανάμεσα στις λέξεις σε κάθε πλάτος οθόνης.

            ΤΙ ΑΛΛΑΖΕΙ. Το πρόβλημα και η απάντηση μένουν πρόζα, μία σειρά η
            καθεμία, σε πλάτος ανάγνωσης. Οι τέσσερις δυνατότητες όμως ΔΕΝ είναι
            πρόζα: είναι λίστα που κάποιος έγραψε ως παράγραφο. Γίνονται κατάστιχο
            δύο στηλών — υποκείμενο αριστερά, τι του συμβαίνει δεξιά — με λεπτή
            γραμμή ανάμεσα. Το μάτι σαρώνει τέσσερις σειρές αντί να διαβάζει
            τέσσερις προτάσεις σε σειρά, και τα ρήματα στοιχίζονται κάθετα. */}
        <p style={{ fontSize: 'clamp(15px,1.55vw,17px)', lineHeight: 1.65, color: MUTED,
          margin: '0 0 12px', maxWidth: 760, textWrap: 'pretty' }}>
          Το φορολογικό πλαίσιο αλλάζει κάθε χρόνο, οι υποχρεώσεις πληθαίνουν, οι προθεσμίες δεν
          περιμένουν. Ε2, Δήλωση Μίσθωσης, ΕΝΦΙΑ, τεκμαρτή έκπτωση, κοινόχρηστα: καθένα με τη δική
          του προθεσμία, και η ευθύνη πάντα πάνω σου.
        </p>
        <p style={{ fontSize: 'clamp(15px,1.55vw,17px)', lineHeight: 1.65, color: MUTED,
          margin: '0 0 clamp(24px,3vw,34px)', maxWidth: 760, textWrap: 'pretty' }}>
        ...
        <dl className="lp-ledger">
          {[
            ['Ο υπολογισμός', 'βγαίνει πριν τον ζητήσεις'],
            ['Η προθεσμία', 'εμφανίζεται πριν τη θυμηθείς'],
            ['Το παραστατικό', 'καταχωρείται με μια φωτογραφία'],
            ['Ο φάκελος του λογιστή', 'αποστέλλεται με ένα κλικ'],
          ].map(([subject, action]) => (
            <div key={subject} className="lp-ledger-row">
              <dt>{subject}</dt>
              <dd>{action}</dd>
            </div>
          ))}
