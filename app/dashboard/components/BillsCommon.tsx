...
// History input — design-system compliant
    ...
    <div style={{ fontFamily: T.font.sans, color: 'var(--text-primary)' }}>
      ...
      {transferMsg && (
        <div style={{
          ...
          <button onClick={() => setTransferMsg(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 16, padding: '0 2px', lineHeight: 1 }}>✕</button>
        </div>
      )}
      ...
      <div style={card}>
        ...
        {/* Info banner */}
        <div style={{
          background: 'var(--bg-elevated)', borderRadius: T.radius.inner, padding: '12px 16px',
          ...
          {([
            ...
            return (
              <div key={opt.key} onClick={() => sMgmt(opt.key)}
                style={{
                  background: isCur ? 'rgba(212,175,66,0.07)' : 'var(--bg-elevated)',
                  border: `1px solid ${isCur ? 'var(--accent)' : 'var(--border-subtle)'}`,
                  borderRadius: T.radius.inner, padding: '10px 12px',
                  cursor: 'pointer', transition: 'all 0.15s', position: 'relative',
                }}>
                {isCur && <div style={{ position: 'absolute', top: 6, right: 8, width: 5, height: 5, borderRadius: '50%', background: 'var(--accent)' }}/>}
                <div style={{ fontSize: 13, fontWeight: 700, fontFamily: T.font.mono, lineHeight: 1, color: isCur ? 'var(--accent)' : 'var(--text-primary)', marginBottom: 3 }}>{opt.cost}</div>
                <div style={{ fontSize: 11, fontFamily: T.font.sans, color: isCur ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{opt.label}</div>
                {opt.url && isCur && (
                  <a href={opt.url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
                    style={{ fontSize: 9, color: 'var(--info)', textDecoration: 'none', fontFamily: T.font.sans, marginTop: 4, display: 'block' }}>
                    Επίσκεψη →
                  </a>
                )}
              </div>
          ...
          <div style={{ background: 'var(--bg-elevated)', borderRadius: T.radius.inner, padding: '14px 18px', border: '1px solid var(--border-subtle)' }}>
            <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' as const }}>
              {[
                ...
                <div key={i}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase' as const, letterSpacing: '0.06em', marginBottom: 4, fontFamily: T.font.sans }}>{k.label}</div>
      ...
      <div style={card}>
        ...
        <div style={{ background: 'var(--bg-elevated)', borderRadius: T.radius.inner, padding: 14, marginBottom: 14, border: '1px solid var(--border-subtle)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: 10, alignItems: 'flex-end' }}>
            ...
            <button onClick={addExtra}
              style={{ background: 'var(--accent)', color: '#000', border: 'none', borderRadius: T.radius.inner, padding: '0 18px', height: 38, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: T.font.sans, marginBottom: 14, whiteSpace: 'nowrap' as const }}>
              + Προσθήκη
            </button>
          </div>
        </div>
        ...
        {extras.length === 0 && (
          <div style={{ textAlign: 'center', padding: 20, color: 'var(--text-tertiary)', fontSize: 11, fontFamily: T.font.sans }}>
            Δεν υπάρχουν καταγεγραμμένες έκτακτες εισφορές
          </div>
        )}

        {extras.map((e, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border-subtle)', opacity: e.transferredToExpenses ? 0.5 : 1 }}>
            <div>
              <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', fontFamily: T.font.sans }}>{e.reason}</span>
              {e.date && <span style={{ fontSize: 10, color: 'var(--text-tertiary)', marginLeft: 10, fontFamily: T.font.sans }}>{new Date(e.date).toLocaleDateString('el-GR')}</span>}
              {e.transferredToExpenses && (
                <span style={{ fontSize: 9, color: 'var(--positive)', marginLeft: 10, background: 'rgba(52,168,83,0.1)', padding: '1px 8px', borderRadius: T.radius.pill, fontFamily: T.font.sans }}>
                  Στις Δαπάνες
                </span>
              )}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button onClick={() => delExtra(i)}
                style={{ width: 26, height: 26, borderRadius: T.radius.badge, border: '1px solid var(--border-subtle)', background: 'transparent', color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 12 }}>
                ✕
              </button>
            </div>
          </div>
      ...
      <div style={card}>
        ...
        <div style={{ position: 'relative', display: 'flex', gap: 4, alignItems: 'flex-end', height: 64, marginBottom: 0, padding: '4px 0 0' }}>
          {monthlyAvg > 0 && (
            <div style={{ position: 'absolute', left: 0, right: 0, bottom: `${(monthlyAvg / maxH) * 56}px`, borderTop: '1px dashed rgba(212,175,66,0.4)', pointerEvents: 'none' }}>
          ...
          {MONTHS_GR.map((m, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                <div style={{ fontSize: 7, fontFamily: T.font.mono, height: 12, display: 'flex', alignItems: 'flex-end', color: isHigh ? 'var(--negative)' : isCur ? 'var(--accent)' : 'var(--text-tertiary)' }}>
                  {val > 0 ? Math.round(val) : ''}
                </div>
                <div style={{ width: '100%', height: `${Math.max(pct * 48, 2)}px`, background: isCur ? 'var(--accent)' : isHigh ? 'var(--negative)' : 'rgba(26,115,232,0.45)', borderRadius: '3px 3px 0 0' }}/>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 14, borderTop: '1px solid var(--border-subtle)', paddingTop: 4 }}>
          {MONTHS_GR.map((m, i) => (
            <div key={i} style={{ flex: 1, fontSize: 8, textAlign: 'center', fontFamily: T.font.sans, color: i === currentMonth ? 'var(--accent)' : 'var(--text-tertiary)', fontWeight: i === currentMonth ? 700 : 400 }}>{m}</div>
          ))}
        </div>

        {/* Input grid — design-system styled inputs */}
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderTop: '2px solid var(--border-subtle)', marginTop: 8 }}>
            ...
            <div style={{ textAlign: 'right' as const }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-tertiary)', fontFamily: T.font.mono }}>{fe(totalCommon * 12)}/έτος</div>
            </div>
          </div>
        </div>
      )}
