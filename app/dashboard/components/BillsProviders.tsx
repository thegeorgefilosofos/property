...
// Internet plans με πραγματικά δεδομένα Ιουνίου 2026
...
// Benchmarks για σύγκριση αγοράς
const BENCHMARKS = {
  internet: { avg: 22.50, label: 'Μ.Ο. Ελλάδας' },
  water:    { avg: 12.00, label: 'Μ.Ο. Αττικής 2π/μήνα=24€' },
...
const DEFAULTS = {
  ...
  internetContract: '24', internetDueDay: '15', internetContractEnd: '', internetSpeedReal: '',
...
export default function BillsProviders({ propertyId, userId = '' }: Props) {
  ...
  const planOptions = (INTERNET_PLANS[s.internetProvider] || []).map(p => ({
    value: p.id, label: `${p.name} — ${p.speed} — ${p.price.toFixed(2)}€/μήνα`,
  }));
  ...
  const secHdr = (label: string, link?: { url: string; text: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 10, borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      {link?.url && <a href={link.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: 'var(--info)', textDecoration: 'none', fontWeight: 600, fontFamily: T.font.sans }}>{link.text}</a>}
    </div>
  );
  ...
  const benchmarkBar = (current: number, avg: number, label: string) => {
    ...
    const avgPct = Math.min(50, 100);
    const isHigh = current > avg * 1.15;
    const isLow  = current < avg * 0.85;
    return (
      <div style={{ marginTop: 10, background: 'var(--bg-elevated)', borderRadius: 8, padding: '10px 14px', border: '1px solid var(--border-subtle)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 10, fontFamily: T.font.sans }}>
          <span style={{ color: 'var(--text-secondary)' }}>Σύγκριση με αγορά — {label}</span>
          <span style={{ fontWeight: 700, color: isHigh ? 'var(--negative)' : isLow ? 'var(--positive)' : 'var(--text-primary)', fontFamily: T.font.mono }}>
            {isHigh ? `+${((current/avg-1)*100).toFixed(0)}% πάνω` : isLow ? `-${((1-current/avg)*100).toFixed(0)}% κάτω` : 'Στο μ.ο.'}
          </span>
        </div>
  ...
  if (loading) return (
    <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: T.font.sans }}>Φόρτωση...</div>
  );
    ...
    <div style={{ fontFamily: T.font.sans, color: 'var(--text-primary)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          { label: 'Αέριο & Security',        value: fe(gasM + securityM),      accent: false },
      ...
      {/* ── Δημοτικά Τέλη ── */}
      <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--accent)', flexShrink: 0 }}/>
          <span style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.06em', color: 'var(--text-secondary)', fontFamily: T.font.sans }}>Δημοτικά Τέλη — Υπολογισμός Ποσοστού</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, alignItems: 'flex-end' }}>
          ...
          <div>
            <div style={{ fontSize: 9, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6, fontFamily: T.font.sans }}>Υπολόγισε από λογαριασμό ρεύματος</div>
            ...
            {s.dimotikaCalcCons && s.dimotikaCalcAmount && parseFloat(s.dimotikaCalcCons) > 0 && (
              <button onClick={() => upd({ dimotika: (parseFloat(s.dimotikaCalcAmount)/parseFloat(s.dimotikaCalcCons)*100).toFixed(1) })}
                style={{ background: 'var(--info)', color: '#fff', border: 'none', borderRadius: 6, padding: '5px 14px', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: T.font.sans }}>
                Εφαρμογή {(parseFloat(s.dimotikaCalcAmount)/parseFloat(s.dimotikaCalcCons)*100).toFixed(1)}%
              </button>
            )}
          </div>
          <div style={{ background: 'var(--bg-base)', borderRadius: 8, padding: 12, textAlign: 'center', border: `1px solid ${s.dimotika ? 'var(--info)' : 'var(--border-subtle)'}` }}>
            <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--info)', fontFamily: T.font.mono }}>{s.dimotika || '—'}{s.dimotika ? '%' : ''}</div>
            <div style={{ fontSize: 9, color: 'var(--text-secondary)', textTransform: 'uppercase', marginTop: 4, fontFamily: T.font.sans }}>Ενεργό ποσοστό</div>
            <div style={{ fontSize: 9, color: 'var(--text-tertiary)', marginTop: 2, fontFamily: T.font.sans }}>Τυπικό: 3-6% · Αθήνα: ~5%</div>
          </div>
        </div>
        <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text-secondary)', fontFamily: T.font.sans }}>
          Το ποσοστό αυτό χρησιμοποιείται αυτόματα στον υπολογισμό του λογαριασμού ρεύματος (tab Ρεύμα).
        </div>
      </div>
      ...
      <div style={card}>
        ...
        {selectedPlan && (
          <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: T.radius.inner, padding: '12px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12, fontSize: 11, fontFamily: T.font.sans }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--info)', flexShrink: 0 }}/>
            <span style={{ color: 'var(--text-secondary)' }}>{selectedPlan.note} · {selectedPlan.hasPhone ? 'Περιλαμβάνει σταθερό τηλέφωνο' : 'Χωρίς σταθερό τηλέφωνο'}</span>
            {provData?.url && <a href={provData.url} target="_blank" rel="noopener noreferrer" style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--accent)', textDecoration: 'none', fontWeight: 600, fontFamily: T.font.sans }}>Επίσημη σελίδα {provData.label}</a>}
          </div>
        )}

        {/* Contract renewal + speedtest */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', paddingBottom: 2 }}>
            {s.internetSpeedReal && s.internetSpeed && (() => {
              const real = parseFloat(s.internetSpeedReal);
              const contract = parseFloat(s.internetSpeed);
              const pct = contract > 0 ? Math.round((real/contract)*100) : 0;
              const good = pct >= 80;
              return (
                <div style={{ background: good?'rgba(52,168,83,0.07)':'rgba(242,153,0,0.07)', border: `1px solid ${good?'rgba(52,168,83,0.25)':'rgba(242,153,0,0.25)'}`, borderRadius: 8, padding: '10px 14px' }}>
                  <div style={{ fontSize: 18, fontWeight: 700, color: good?'var(--positive)':'var(--warning)', fontFamily: T.font.mono }}>{pct}%</div>
                  <div style={{ fontSize: 9, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: T.font.sans }}>{good?'Καλή απόδοση':'Μειωμένη ταχύτητα'}</div>
                </div>
              );
            })()}
            {(!s.internetSpeedReal || !s.internetSpeed) && (
              <div style={{ fontSize: 10, color: 'var(--text-tertiary)', fontFamily: T.font.sans }}>Μέτρησε τη ταχύτητά σου στο <a href="https://www.speedtest.net" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', textDecoration: 'none' }}>speedtest.net</a></div>
            )}
          </div>
        </div>
        ...
        {s.internetContractEnd && (() => {
          ...
          return (
            <div style={{ background: days<=14?'rgba(197,34,31,0.07)':'rgba(242,153,0,0.07)', border: `1px solid ${days<=14?'rgba(197,34,31,0.25)':'rgba(242,153,0,0.25)'}`, borderRadius: T.radius.inner, padding: '10px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, fontFamily: T.font.sans }}>
              ...
              <span style={{ color: 'var(--text-secondary)' }}>Λήξη συμβολαίου Internet σε <strong style={{ color: days<=14?'var(--negative)':'var(--warning)' }}>{days} ημέρες</strong> — Σύγκρινε στο ΕΕΤΤ 360° για καλύτερη τιμή.</span>
            </div>
          );
        })()}
        ...
        {s.internetPhone && (
          <div style={{ background: 'var(--bg-elevated)', borderRadius: T.radius.inner, padding: 14, marginBottom: 12, border: '1px solid var(--border-subtle)' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10, fontFamily: T.font.sans }}>Τι περιλαμβάνει το σταθερό τηλέφωνο</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
              {[
                ...
                <div key={f.key} onClick={() => upd({ [f.key]: !f.val } as any)}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, background: f.val ? 'rgba(52,168,83,0.08)' : 'var(--bg-base)', border: `1px solid ${f.val ? 'var(--positive)' : 'var(--border-subtle)'}`, borderRadius: 8, padding: '6px 14px', cursor: 'pointer', transition: 'all 0.15s' }}>
        ...
        {INTERNET_PLANS[s.internetProvider]?.length > 0 && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10, fontFamily: T.font.sans }}>Διαθέσιμα Προγράμματα {provData?.label}</div>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                <thead>
                  <tr>
                    {['Πρόγραμμα','Ταχύτητα','Σταθερό Τηλ.','Δέσμευση','Μηνιαίο','Ετήσιο'].map((h, i) => (
                      <th key={i} style={{ fontSize: 9, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-secondary)', padding: '6px 10px', borderBottom: '1px solid var(--border-subtle)', textAlign: 'left', fontWeight: 600, fontFamily: T.font.sans, background: 'var(--bg-elevated)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {(INTERNET_PLANS[s.internetProvider] || []).map(plan => {
                    ...
                    return (
                      <tr key={plan.id}
                        ...
                        <td style={{ padding: '7px 10px', color: plan.hasPhone ? 'var(--positive)' : 'var(--text-tertiary)', fontWeight: 700, textAlign: 'center' }}>{plan.hasPhone ? '✓' : '—'}</td>
                        <td style={{ padding: '7px 10px', color: 'var(--text-tertiary)', fontSize: 10, fontFamily: T.font.sans }}>{plan.note}</td>
                        <td style={{ padding: '7px 10px', fontWeight: 600, color: isCur ? 'var(--accent)' : 'var(--text-primary)', fontFamily: T.font.mono }}>{fe(plan.price)}</td>
                        <td style={{ padding: '7px 10px', color: 'var(--text-tertiary)', fontFamily: T.font.mono, fontSize: 10 }}>{fe(plan.price * 12)}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <div style={{ marginTop: 16, borderTop: '1px solid var(--border-subtle)', paddingTop: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--accent)', flexShrink: 0 }}/>
            <span style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.06em', color: 'var(--text-secondary)', fontFamily: T.font.sans, flex: 1 }}>Pay TV</span>
          ...
          {s.hasTV && (
            <div style={g4}>
              ...
              <div style={{ paddingTop: 22 }}>
                <Toggle on={s.tvHasSports} onChange={v => upd({ tvHasSports: v })} label="Sports Package" labelOff="Χωρίς Sports"/>
              </div>
            </div>
          )}
        </div>
      ...
      <div style={card}>
        ...
        <div style={g4}>
          ...
          <NumberInput label="Διμηνιαίος Λογαριασμός (€)" value={s.waterBiMonthly}
            onChange={v => upd({ waterBiMonthly: v, waterMonthly: v ? String((parseFloat(v)||0)/2) : '' })} suffix="€" step={5}/>
        ...
        {waterM > 0 && (
          <div style={{ background: 'var(--bg-elevated)', borderRadius: 8, padding: '10px 14px', fontSize: 11, color: 'var(--text-secondary)', fontFamily: T.font.sans, border: '1px solid var(--border-subtle)' }}>
            Μηνιαίο: <strong style={{ color: 'var(--text-primary)', fontFamily: T.font.mono }}>{fe(waterM)}</strong>
            {s.waterPersons && <span style={{ marginLeft: 14 }}>Ανά άτομο: <strong style={{ fontFamily: T.font.mono }}>{fe(waterM/parseInt(s.waterPersons))}</strong>/μήνα</span>}
        ...
        {benchmarkBar(waterM * 2, 24, 'Μ.Ο. διμηνιαίος ΕΥΔΑΠ Αττικής')}
      </div>
      ...
      <div style={card}>
        ...
        {heatingM > 0 && (
          <div style={{ background: 'var(--bg-elevated)', borderRadius: 8, padding: '12px 16px', fontSize: 11, color: 'var(--text-secondary)', fontFamily: T.font.sans, border: '1px solid var(--border-subtle)' }}>
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', marginBottom: 8 }}>
              <div>
                <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4, fontFamily: T.font.sans }}>Μέσο Μηνιαίο</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', fontFamily: T.font.mono }}>{fe(heatingM)}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4, fontFamily: T.font.sans }}>Εκτιμώμενο Ετήσιο</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', fontFamily: T.font.mono }}>{fe(heatingM*12)}</div>
              </div>
              {s.heatingType==='autonomous_oil' && s.heatingLitersPerYear && (
                <div>
                  <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4, fontFamily: T.font.sans }}>Κόστος/kWh</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', fontFamily: T.font.mono }}>{(parseFloat(s.heatingOilPricePerLiter)/10.5).toFixed(4)} €</div>
                </div>
              )}
            </div>
            ...
            <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 8, fontSize: 10, color: 'var(--text-tertiary)', fontFamily: T.font.sans }}>
              Μ.Ο. αγοράς: Αέριο ~0.08€/kWh · Πετρέλαιο ~0.10€/kWh · Αντλία θερμότητας ~0.06€/kWh — Αθήνα 2026
            </div>
          </div>
        )}
      ...
      {/* ── Αέριο ── */}
      <div style={card}>
        {secHdr('Αέριο', { url: 'https://energycost.gr', text: 'ΡΑΑΕΥ energycost.gr' })}
        <div style={g3}>
        ...
        {gasM > 0 && (
          <div style={{ background: 'var(--bg-elevated)', borderRadius: 8, padding: '10px 14px', fontSize: 11, color: 'var(--text-secondary)', fontFamily: T.font.sans, border: '1px solid var(--border-subtle)' }}>
            Ετήσιο κόστος αερίου: <strong style={{ color: 'var(--text-primary)', fontFamily: T.font.mono }}>{fe(gasM*12)}</strong>
      ...
      {totalM > 0 && (
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 10, borderBottom: '1px solid var(--border-subtle)' }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--accent)', flexShrink: 0 }}/>
            <span style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.06em', color: 'var(--text-secondary)', fontFamily: T.font.sans }}>Σύνοψη Παρόχων</span>
          </div>
          {[
            { label: 'Internet',  amount: internetCost, skip: !internetCost },
            { label: s.tvProvider === 'cosmote' ? 'Cosmote TV' : 'Pay TV', amount: tvCost, skip: !s.hasTV },
            { label: 'Νερό',     amount: waterM,   skip: !waterM   },
            { label: 'Θέρμανση', amount: heatingM, skip: !heatingM },
            { label: 'Αέριο',   amount: gasM,     skip: !gasM     },
