...
// ΕΝΦΙΑ 2026 συντελεστές ζώνης (ΦΕΚ 2026)
...
export default function BillsServices({ propertyId, userId = '' }: Props) {
  ...
  const poolM     = s.hasPool
    ? ((parseFloat(s.poolWeeklyCost)||0)*4.33+(parseFloat(s.poolChemicals)||0)) : 0;
  ...
  if (loading) return (
    <div style={{ padding:40, textAlign:'center', color:'var(--text-tertiary)', fontSize:11,
      textTransform:'uppercase', letterSpacing:'0.1em', fontFamily:T.font.sans }}>Φόρτωση...</div>
  );
    ...
    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16,
      ...
      <div>
        <div style={{ fontSize:11, fontWeight:600, color:'var(--text-secondary)',
          textTransform:'uppercase' as const, letterSpacing:'0.06em', fontFamily:T.font.sans }}>{label}</div>
  ...
  const svcHdr = (label: string, active: boolean, onToggle: (v:boolean)=>void, cost?: number) => (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
      marginBottom: active ? 16 : 0, paddingBottom: active ? 10 : 0,
      borderBottom: active ? '1px solid var(--border-subtle)' : 'none' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        <div style={{ width:6, height:6, borderRadius:'50%',
          background: active ? 'var(--accent)' : 'var(--border-default)', flexShrink:0 }}/>
        <div>
          <span style={{ fontSize:11, fontWeight:600, textTransform:'uppercase' as const,
            letterSpacing:'0.06em', color: active ? 'var(--text-primary)' : 'var(--text-secondary)',
            fontFamily:T.font.sans }}>{label}</span>
          {active && cost && cost > 0 && (
            <span style={{ fontSize:11, color:'var(--text-tertiary)', fontFamily:T.font.mono, marginLeft:8 }}>
              {fe(cost)}/μήνα
            </span>
          )}
        </div>
  ...
  return (
    <div style={{ fontFamily:T.font.sans, color:'var(--text-primary)' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:16 }}>
        {([
          ...
          <div key={i} style={{ background:'var(--bg-elevated)', border:'1px solid var(--border-subtle)',
            borderRadius:T.radius.card, padding:'16px 18px' }}>
            <div style={{ fontSize:10, fontWeight:600, color:'var(--text-tertiary)', textTransform:'uppercase' as const,
              letterSpacing:'0.06em', marginBottom:10, fontFamily:T.font.sans }}>{k.label}</div>
            <div style={{ fontSize:22, fontWeight:700, color:'var(--text-primary)',
              fontFamily:T.font.mono, lineHeight:1 }}>{k.value}</div>
          </div>
        ))}
      </div>

      {/* ── ΕΝΦΙΑ 2026 ──────────────────────────────────────────────────── */}
      <div style={{ ...card, borderTop:'2px solid var(--info)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div>
              <div style={{ fontSize:11, fontWeight:600, color:'var(--text-secondary)',
                textTransform:'uppercase' as const, letterSpacing:'0.06em', fontFamily:T.font.sans }}>
                ΕΝΦΙΑ 2026 — Υπολογιστής
              </div>
              <div style={{ fontSize:10, color:'var(--text-tertiary)', fontFamily:T.font.sans, marginTop:1 }}>
                Εκτίμηση βάσει Ε9 — επαλήθευσε στο myAADE.gr
              </div>
            </div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <span style={{ fontSize:9, fontWeight:700, color:'var(--warning)',
              background:'rgba(242,153,0,0.08)', padding:'2px 8px',
              borderRadius:T.radius.pill, fontFamily:T.font.sans,
              border:'1px solid rgba(242,153,0,0.2)', textTransform:'uppercase',
              letterSpacing:'0.04em' }}>Εκτίμηση</span>
            <a href="https://www.aade.gr/polites/forologikes-ypiresies/miaadem"
              target="_blank" rel="noopener noreferrer"
              style={{ display:'inline-flex', alignItems:'center', gap:4,
                fontSize:10, color:'var(--info)', fontFamily:T.font.sans,
                fontWeight:600, textDecoration:'none', padding:'2px 10px',
                background:'rgba(26,115,232,0.06)', borderRadius:T.radius.pill,
                border:'1px solid rgba(26,115,232,0.18)' }}>
              myAADE.gr
            </a>
            <button onClick={()=>upd({enfiaShowCalc:!s.enfiaShowCalc})}
              style={{ fontSize:10, color:'var(--text-tertiary)', background:'transparent',
                border:'1px solid var(--border-subtle)', borderRadius:T.radius.badge,
                padding:'2px 10px', cursor:'pointer', fontFamily:T.font.sans,
        ...
        {enfiaChecklist && (
          <div style={{ background:enfiaChecklist.status==='done'?'rgba(52,168,83,0.07)':'rgba(26,115,232,0.06)',
            border:`1px solid ${enfiaChecklist.status==='done'?'rgba(52,168,83,0.2)':'rgba(26,115,232,0.15)'}`,
            borderRadius:T.radius.inner, padding:'9px 16px', marginBottom:14,
            display:'flex', alignItems:'center', gap:10, fontSize:12, fontFamily:T.font.sans }}>
            <div style={{ width:6, height:6, borderRadius:'50%',
              background:enfiaChecklist.status==='done'?'var(--positive)':'var(--info)', flexShrink:0 }}/>
            ...
            <span style={{ fontSize:10, color:'var(--text-tertiary)', background:'var(--bg-elevated)',
              padding:'2px 8px', borderRadius:T.radius.pill }}>Checklist</span>
          </div>
        )}

        {/* 2026 info banner */}
        <div style={{ background:'rgba(26,115,232,0.05)', border:'1px solid rgba(26,115,232,0.12)',
          borderRadius:T.radius.inner, padding:'10px 16px', marginBottom:16,
          ...
          <div style={{ fontSize:11, color:'var(--text-secondary)', lineHeight:1.6, fontFamily:T.font.sans }}>
            ...
            <a href="https://www.taxheaven.gr" target="_blank" rel="noopener noreferrer"
              style={{ color:'var(--accent)', textDecoration:'none', fontWeight:600 }}>Ανάλυση →</a>
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:6, marginBottom:20 }}>
          {ENFIA_DEADLINES.map((d,i) => {
            ...
            return (
              <div key={i} style={{
                background: isNext ? 'rgba(212,175,66,0.1)' : isPast ? 'var(--bg-elevated)' : 'var(--bg-surface)',
                border: `1px solid ${isNext ? 'var(--accent)' : 'var(--border-subtle)'}`,
                borderRadius: T.radius.inner, padding:'10px 6px', textAlign:'center' as const,
                opacity: isPast ? 0.45 : 1, transition:'all 0.15s',
              }}>
                <div style={{ fontSize:8, fontWeight:700, fontFamily:T.font.sans, marginBottom:3,
                  textTransform:'uppercase' as const, letterSpacing:'0.05em',
                  color: isPast ? 'var(--positive)' : isNext ? 'var(--accent)' : 'transparent',
                  minHeight:12 }}>
                  {isPast ? 'ΠΛΗΡ.' : isNext ? 'ΕΠΟΜ.' : ''}
                </div>
                <div style={{ fontSize:11, fontWeight:isNext?700:500,
                  color:isNext?'var(--accent)':'var(--text-primary)', fontFamily:T.font.sans }}>{d.label}</div>
                <div style={{ fontSize:10, color:isNext?'var(--accent)':'var(--text-tertiary)',
                  fontFamily:T.font.mono, marginTop:2 }}>{d.month}</div>
                {isNext && dLeft>=0 && dLeft<=90 && (
                  <div style={{ fontSize:9, color:'var(--accent)', fontFamily:T.font.sans,
                    marginTop:3, fontWeight:700 }}>{dLeft} ημ.</div>
                )}
              </div>
            );
          })}
        ...
        {nextDeadline && daysToDeadline!==null && daysToDeadline<=30 && (
          <div style={{ background:daysToDeadline<=7?'rgba(197,34,31,0.07)':'rgba(242,153,0,0.07)',
            border:`1px solid ${daysToDeadline<=7?'rgba(197,34,31,0.25)':'rgba(242,153,0,0.25)'}`,
            borderRadius:T.radius.inner, padding:'10px 16px', marginBottom:16,
            display:'flex', alignItems:'center', gap:10, fontSize:12, fontFamily:T.font.sans }}>
            <div style={{ width:6, height:6, borderRadius:'50%',
              background:daysToDeadline<=7?'var(--negative)':'var(--warning)', flexShrink:0 }}/>
            <span style={{ color:'var(--text-primary)', fontWeight:500 }}>
              {nextDeadline.label} ΕΝΦΙΑ 2026{' '}
              <span style={{ color:daysToDeadline<=7?'var(--negative)':'var(--warning)', fontWeight:700 }}>
                σε {daysToDeadline} ημέρες
              </span>
        ...
        {s.enfiaShowCalc && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
            ...
            <div>
              <div style={g2}>
                <NumberInput label="Εμβαδόν (τ.μ.)" value={s.enfiaSqm}
                  onChange={v=>upd({enfiaSqm:v})} suffix="τ.μ."/>
                <NumberInput label="Ποσοστό Ιδιοκτησίας (%)" value={s.enfiaOwnership}
                  onChange={v=>upd({enfiaOwnership:v})} suffix="%" max={100}/>
              </div>
              <div style={{ marginBottom:14 }}>
                <CustomSelect label="Τιμή Ζώνης (€/τ.μ.)" value={s.enfiaZone}
                  onChange={v=>upd({enfiaZone:v})} options={ZONE_OPTIONS}/>
              </div>
              <div style={g2}>
                <CustomSelect label="Όροφος" value={s.enfiaFloor}
                  onChange={v=>upd({enfiaFloor:v})} options={FLOOR_OPTIONS}/>
                <CustomSelect label="Παλαιότητα" value={s.enfiaAge}
                  onChange={v=>upd({enfiaAge:v})} options={AGE_OPTIONS}/>
              </div>
              <div style={{ marginBottom:14 }}>
                <NumberInput label="Συνολική Αξία Ακινήτων (€) — για Συμπληρωματικό Φόρο"
                  value={s.enfiaTotalVal} onChange={v=>upd({enfiaTotalVal:v})} suffix="€"/>
              </div>

              {/* Μειώσεις */}
              <div style={{ fontSize:10, fontWeight:600, color:'var(--text-secondary)',
                textTransform:'uppercase' as const, letterSpacing:'0.06em',
                marginBottom:8, fontFamily:T.font.sans }}>Μειώσεις</div>
                  ...
                  return (
                    <div key={r.key} onClick={()=>toggleReduction(r.key)}
                      style={{ display:'flex', alignItems:'center', gap:10,
                        ...
                        background: active ? 'rgba(52,168,83,0.07)' : 'var(--bg-elevated)',
                        border: `1px solid ${active ? 'var(--positive)' : 'var(--border-subtle)'}`,
                        borderRadius:T.radius.inner }}>
                      <div style={{ width:16, height:16, borderRadius:4, flexShrink:0,
                        border:`2px solid ${active?'var(--positive)':'var(--border-default)'}`,
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:12, color:'var(--text-primary)',
                          fontFamily:T.font.sans, fontWeight: active?600:400 }}>{r.label}</div>
                        <div style={{ fontSize:10, color:'var(--text-tertiary)',
                          fontFamily:T.font.sans, marginTop:1 }}>{r.note}</div>
                      </div>
                      <span style={{ fontSize:11, fontWeight:700, color:'var(--positive)',
                        fontFamily:T.font.mono }}>-{r.pct}%</span>
                    </div>
                  );
                })}
              </div>
              ...
              {!s.enfiaZone && (
                <div style={{ marginTop:14, display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                  <NumberInput label="ΕΝΦΙΑ/έτος (χειροκίνητα)" value={s.enfiaAnnual}
                    ...
                    suffix="€" step={50}/>
                  <div style={{ background:'var(--bg-elevated)', borderRadius:T.radius.inner,
                    padding:'12px 14px', border:'1px solid var(--border-subtle)' }}>
                    <div style={{ fontSize:10, fontWeight:600, color:'var(--text-tertiary)',
                      textTransform:'uppercase' as const, letterSpacing:'0.06em',
                      marginBottom:6, fontFamily:T.font.sans }}>Μηνιαία Αναγωγή</div>
                    <div style={{ fontSize:18, fontWeight:700, color:'var(--text-primary)',
                      fontFamily:T.font.mono }}>{enfiaM>0?fe(enfiaM):'—'}</div>
                  </div>
                </div>
              )}
            </div>
              ...
              {enfiaResult && enfiaResult.final > 0 ? (
                <>
                  ...
                  <div style={{ background:'var(--bg-elevated)', border:'1px solid var(--border-subtle)',
                    borderRadius:T.radius.card, padding:18, marginBottom:14 }}>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
                      <div>
                        <div style={{ fontSize:10, fontWeight:600, color:'var(--text-tertiary)',
                          textTransform:'uppercase' as const, letterSpacing:'0.06em',
                          marginBottom:6, fontFamily:T.font.sans }}>Τελικός ΕΝΦΙΑ</div>
                        <div style={{ fontSize:28, fontWeight:700, color:'var(--negative)',
                          fontFamily:T.font.mono, lineHeight:1 }}>{fe(enfiaResult.final,0)}</div>
                      </div>
                      <div>
                        <div style={{ fontSize:10, fontWeight:600, color:'var(--text-tertiary)',
                          textTransform:'uppercase' as const, letterSpacing:'0.06em',
                          marginBottom:6, fontFamily:T.font.sans }}>Δόση (~6 δόσεις)</div>
                        <div style={{ fontSize:28, fontWeight:700, color:'var(--warning)',
                          fontFamily:T.font.mono, lineHeight:1 }}>{fe(enfiaResult.installment,0)}</div>
                      </div>
                    </div>
                    ...
                    {[
                      ...
                      <div key={i} style={{ display:'flex', justifyContent:'space-between',
                        ...
                        <span style={{ fontSize:11, color:'var(--text-secondary)',
                          fontFamily:T.font.sans }}>{row.label}</span>
                        <span style={{ fontSize:12, fontWeight:600, fontFamily:T.font.mono,
                          color:row.pos?'var(--positive)':'var(--text-primary)' }}>
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between',
                      ...
                      <span style={{ fontSize:12, fontWeight:600, color:'var(--text-primary)',
                        fontFamily:T.font.sans }}>Σύνολο ΕΝΦΙΑ</span>
                      <span style={{ fontSize:20, fontWeight:700, color:'var(--negative)',
                        fontFamily:T.font.mono }}>{fe(enfiaResult.final)}</span>
                    </div>
                  </div>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6,
                    ...
                    {ENFIA_DEADLINES.map((d,i) => {
                      ...
                      return (
                        <div key={i} style={{
                          background: isNext?'rgba(212,175,66,0.1)':'var(--bg-elevated)',
                          border:`1px solid ${isNext?'var(--accent)':'var(--border-subtle)'}`,
                          borderRadius:T.radius.inner, padding:'10px 12px', opacity:isPast?0.45:1 }}>
                          <div style={{ fontSize:9, fontWeight:600, fontFamily:T.font.sans,
                            textTransform:'uppercase' as const, letterSpacing:'0.06em', marginBottom:4,
                            color:isPast?'var(--positive)':isNext?'var(--accent)':'var(--text-tertiary)' }}>
                            {d.label}{isPast?' ✓':''}
                          </div>
                          <div style={{ fontSize:15, fontWeight:700, fontFamily:T.font.mono, lineHeight:1,
                            color:isNext?'var(--accent)':isPast?'var(--positive)':'var(--text-primary)' }}>
                            {fe(enfiaResult.installment,0)}
                          </div>
                          <div style={{ fontSize:9, color:'var(--text-tertiary)',
                            fontFamily:T.font.sans, marginTop:3 }}>{d.month}</div>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {!(s.enfiaReductions||[]).includes('insurance') && (
                    <div style={{ background:'rgba(52,168,83,0.07)',
                      border:'1px solid rgba(52,168,83,0.2)', borderRadius:T.radius.inner,
                      padding:'10px 14px', fontSize:11, color:'var(--positive)',
                ...
                <div style={{ background:'var(--bg-elevated)', border:'1px solid var(--border-subtle)',
                  borderRadius:T.radius.card, padding:24, textAlign:'center' as const }}>
                  <div style={{ fontSize:13, fontWeight:600, color:'var(--text-primary)',
                    fontFamily:T.font.sans, marginBottom:8 }}>Συμπλήρωσε εμβαδόν + τιμή ζώνης</div>
                  <div style={{ fontSize:11, color:'var(--text-tertiary)', fontFamily:T.font.sans,
                    marginBottom:18, lineHeight:1.5 }}>
                    Η τιμή ζώνης βρίσκεται στο myAADE.gr → Ε9
                  </div>
                  <a href="https://www1.aade.gr/saadeweb/menu.aspx" target="_blank" rel="noopener noreferrer"
                    style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:12,
                      color:'var(--info)', background:'rgba(26,115,232,0.08)',
                      padding:'8px 18px', borderRadius:T.radius.pill,
                      border:'1px solid rgba(26,115,232,0.2)', textDecoration:'none',
                      fontWeight:600, fontFamily:T.font.sans }}>
                    Άνοιγμα myAADE.gr →
                  </a>
                </div>
              )}
      ...
      <div style={card}>
        ...
        <div style={{ background:'var(--bg-elevated)', borderRadius:T.radius.inner,
          padding:14, marginBottom:14, border:'1px solid var(--border-subtle)' }}>
          <div style={{ fontSize:10, fontWeight:600, color:'var(--text-secondary)',
            marginBottom:10, fontFamily:T.font.sans }}>
            Υπολογισμός ποσοστού από τελευταίο λογαριασμό ρεύματος
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:12, alignItems:'flex-end' }}>
            <NumberInput label="Σύνολο λογαριασμού (€)" value={s.lastBillTotal}
              onChange={v=>upd({lastBillTotal:v})} suffix="€" step={1}/>
            <NumberInput label="Δημοτικά στον λογαριασμό (€)" value={s.lastBillDimotika}
              onChange={v=>upd({lastBillDimotika:v})} suffix="€" step={0.5}/>
            <div style={{ background:'var(--bg-surface)',
              border:`1px solid ${dimotikaPct>0?'var(--accent)':'var(--border-subtle)'}`,
              borderRadius:T.radius.inner, padding:'12px 16px', textAlign:'center' as const,
              marginBottom:14, minWidth:90 }}>
              <div style={{ fontSize:20, fontWeight:700, color:'var(--accent)',
                fontFamily:T.font.mono, lineHeight:1 }}>{dimotikaPct>0?`${dimotikaPct.toFixed(1)}%`:'—'}</div>
              <div style={{ fontSize:9, color:'var(--text-tertiary)', textTransform:'uppercase' as const,
                fontFamily:T.font.sans, marginTop:3 }}>Ποσοστό</div>
            </div>
          </div>
        </div>

        <div style={{ fontSize:10, fontWeight:600, color:'var(--text-secondary)',
          textTransform:'uppercase' as const, letterSpacing:'0.06em', marginBottom:10, fontFamily:T.font.sans }}>
        ...
        <div style={{ display:'flex', gap:4, alignItems:'flex-end', height:56, marginBottom:4, padding:'4px 0 0' }}>
          {MONTHS_GR.map((m,i) => {
            ...
            return (
              <div key={i} style={{ flex:1, display:'flex', flexDirection:'column' as const,
                alignItems:'center', gap:1 }}>
                <div style={{ fontSize:7, color:isHigh?'var(--negative)':isCur?'var(--accent)':'var(--text-tertiary)',
                  fontFamily:T.font.mono, height:12, display:'flex', alignItems:'flex-end' }}>
                  {val>0?Math.round(val):''}
                </div>
                <div style={{ width:'100%', height:`${Math.max(pct*42,2)}px`,
                  background:isCur?'var(--accent)':isHigh?'var(--negative)':'rgba(26,115,232,0.45)',
        ...
        <div style={{ display:'flex', gap:4, marginBottom:14,
          borderTop:'1px solid var(--border-subtle)', paddingTop:4 }}>
          {MONTHS_GR.map((m,i) => (
            <div key={i} style={{ flex:1, fontSize:8,
              color:i===currentMonth?'var(--accent)':'var(--text-tertiary)',
              textAlign:'center' as const, fontWeight:i===currentMonth?700:400,
              fontFamily:T.font.sans }}>{m}</div>
          ))}
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:6 }}>
          {MONTHS_GR.map((m,i) => (
            <div key={i}>
              <label style={{ fontSize:9, color:'var(--text-secondary)', display:'block',
                marginBottom:4, textAlign:'center' as const, fontFamily:T.font.sans }}>{m}</label>
              <input type="number" value={s.dimotikaHistory[i]} onChange={e=>updHistory(i,e.target.value)}
                placeholder="€"
                style={{ width:'100%', background:'var(--bg-elevated)',
                  ...
                  fontSize:11, fontFamily:T.font.mono, outline:'none',
      ...
      <div style={card}>
        ...
        {s.hasCleaning && (
          <>
            ...
            {cleaningM>0 && (
              <div style={{ background:'var(--bg-elevated)', borderRadius:T.radius.inner,
                padding:'10px 14px', fontSize:11, color:'var(--text-secondary)',
                fontFamily:T.font.sans, border:'1px solid var(--border-subtle)' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:12, alignItems:'flex-end' }}>
            ...
            <button onClick={addOther}
              style={{ background:'var(--accent)', color:'#000', border:'none',
                borderRadius:T.radius.btn, padding:'0 20px', height:38, fontSize:12,
                fontWeight:700, cursor:'pointer', fontFamily:T.font.sans, marginBottom:14, whiteSpace:'nowrap' }}>
              + Προσθήκη
            </button>
          </div>
        </div>
        {(s.otherServices||[]).map((o:any,i:number) => (
          <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
            ...
            <div>
              ...
              <span style={{ fontSize:9, color:'var(--text-tertiary)', marginLeft:10, fontFamily:T.font.sans }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <button onClick={()=>delOther(i)}
                style={{ width:26, height:26, borderRadius:T.radius.badge,
                  border:'1px solid var(--border-subtle)', background:'transparent',
                  color:'var(--text-tertiary)', cursor:'pointer', fontSize:12 }}>✕</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {totalServices > 0 && (
        <div style={card}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
            padding:'14px 0', borderTop:'2px solid var(--border-subtle)', marginTop:8 }}>
            <span style={{ fontSize:13, fontWeight:700, fontFamily:T.font.sans }}>Σύνολο Υπηρεσιών</span>
            <div style={{ textAlign:'right' as const }}>
              <div style={{ fontSize:22, fontWeight:700, color:'var(--text-primary)',
                fontFamily:T.font.mono, lineHeight:1 }}>{fe(totalServices)}/μήνα</div>
