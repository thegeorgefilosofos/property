      ...
      <Card className="acc-section" style={{ animationDelay: '0ms' }}>
        ...
        <div {...fixedCols(COMPARED.length, 12, 'stretch')}>
          {COMPARED.map(id => {
            ...
            // Το «Αναβάθμιση» καλούσε τον χειριστή που ΑΝΟΙΓΕΙ αυτή τη σύγκριση,
            // η οποία, για να διαβαστεί το κουμπί, είναι ήδη ανοιχτή. Το μόνο
            // ορατό αποτέλεσμα ήταν κύλιση προς τα πάνω. Χρέωση με κάρτα δεν
            // υπάρχει ακόμη (βλ. Billing.tsx), άρα το κουμπί δεν μπορεί να
            // αναβαθμίσει· μπορεί όμως να πάει εκεί όπου συνεχίζει πραγματικά ο
            // χρήστης, στη χρέωση από κάτω. Το λεκτικό λέει τον προορισμό, και
            // μένει κοντό: στα 360 εικονοστοιχεία η σειρά σπάει σε δύο στήλες
            // και το κουμπί έχει περίπου 124 να χωρέσει, όχι 160.
            const cta = isCurrent || locked || colRank <= curRank
              ? null
              : <Btn variant="primary" onClick={() => onUpgrade?.()}>Στη χρέωση</Btn>;
