        ...
        <div className="cmdk-list" ref={listRef}>
          {rows.length === 0 ? (
            ...
            groups.map((g, gi) => (
              <div key={gi}>
                {g.label && (
                  <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-tertiary)', padding: '10px 16px 4px', fontFamily: T.font.sans }}>{g.label}</div>
                )}
                {g.items.map(it => {
                  idx++;
                  const i = idx;
