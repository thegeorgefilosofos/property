...
export default function BillsElectricity({propertyId,userId}:{propertyId:string;userId?:string}) {
  ...
  const setShowCustom = (v:boolean|((_:boolean)=>boolean)) => updateSettings({ showCustom: typeof v==='function'?v(settings.showCustom):v });
  ...
  const setVnmEnabled = (v:boolean|((_:boolean)=>boolean)) => updateSettings({ vnmEnabled: typeof v==='function'?v(settings.vnmEnabled):v });
    ...
    <div style={{fontFamily:T.font.sans, color:'var(--text-primary)'}}>
      ...
      {insData && (insData.eq || insData.fl) && (
        <div style={{background:'rgba(52,168,83,0.07)',border:'1px solid rgba(52,168,83,0.25)',borderRadius:10,padding:'11px 18px',marginBottom:14,display:'flex',alignItems:'center',gap:12}}>
          ...
          <span style={{fontSize:10,color:'var(--text-tertiary)',background:'var(--bg-elevated)',padding:'2px 10px',borderRadius:20,whiteSpace:'nowrap',fontFamily:T.font.sans}}>από Ασφάλεια & Συνδρομές</span>
        </div>
      )}
      ...
      <div style={{background:'rgba(26,115,232,0.06)',border:'1px solid rgba(26,115,232,0.2)',borderRadius:10,padding:'11px 18px',marginBottom:14,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
        ...
        <a href={RAAYEY_URL} target="_blank" rel="noopener noreferrer"
          style={{background:'var(--info)',color:'#fff',border:'none',borderRadius:8,padding:'8px 16px',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:T.font.sans,textDecoration:'none',whiteSpace:'nowrap'}}>
          Σύγκριση Τώρα
        </a>
      </div>
      ...
      <div style={card}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
                {[
                  ...
                  <div key={i}>
                    ...
                    <div style={{fontSize:9,color:'var(--text-secondary)',textTransform:'uppercase',fontFamily:T.font.sans}}>{k.label}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{background:'rgba(26,115,232,0.05)',border:'1px solid rgba(26,115,232,0.15)',borderRadius:10,padding:14,marginBottom:12}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,alignItems:'flex-end'}}>
            ...
            <div>
              <div style={{fontSize:9,color:'var(--text-secondary)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:6,fontFamily:T.font.sans}}>Υπολόγισε από λογαριασμό</div>
              ...
              {dimotikaCalcCons&&dimotikaCalcAmt&&parseFloat(dimotikaCalcCons)>0&&(
                <button onClick={()=>setDimotika((parseFloat(dimotikaCalcAmt)/parseFloat(dimotikaCalcCons)*100).toFixed(1))}
                  style={{background:'var(--info)',color:'#fff',border:'none',borderRadius:6,padding:'5px 12px',fontSize:10,fontWeight:700,cursor:'pointer',fontFamily:T.font.sans}}>
                  Εφαρμογή {(parseFloat(dimotikaCalcAmt)/parseFloat(dimotikaCalcCons)*100).toFixed(1)}%
                </button>
              )}
            </div>
            <div style={{background:'var(--bg-base)',borderRadius:8,padding:12,textAlign:'center',border:`1px solid ${dimotika?'var(--info)':'var(--border-subtle)'}`}}>
          ...
          {MONTHS_GR.map((m,i)=>(
            <div key={i}>
              ...
              <input type="number" value={kwhHistory[i]} onChange={e=>{const n=[...kwhHistory];n[i]=e.target.value;setKwhHistory(n);}}
                style={{width:'100%',background:'var(--bg-base)',border:`1px solid ${i===currentMonth?'var(--accent)':'var(--border-subtle)'}`,borderRadius:6,padding:'6px 4px',color:'var(--text-primary)',fontSize:11,fontFamily:T.font.mono,outline:'none',textAlign:'center',boxSizing:'border-box'}}/>
            </div>
          ))}
        </div>
        {calc&&(
      ...
      <div style={card}>
            ...
            <thead>
              <tr>
                {['Πάροχος','Τιμολόγιο','Τύπος','Τιμή kWh','Εκτ. Μηνιαίο','Εκτ. Ετήσιο','Διαφορά/έτος'].map((h,i)=>(
                  <th key={i} style={{fontSize:9,letterSpacing:'0.06em',textTransform:'uppercase',color:'var(--text-secondary)',padding:'6px 8px',borderBottom:'1px solid var(--border-subtle)',textAlign:'left',fontWeight:600,fontFamily:T.font.sans,background:'var(--bg-elevated)'}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {PROVIDERS.flatMap(p=>p.tariffs.map(t=>{
                ...
                return(
                  <tr key={tv.id} onClick={()=>{setProvider(p.value);setTariffId(tv.id);}}
                    ...
                    <td style={{padding:'8px',fontWeight:isCur?700:400,color:isCur?'var(--accent)':'var(--text-primary)',whiteSpace:'nowrap',fontFamily:T.font.sans}}>
        ...
        <div style={{marginTop:10,fontSize:9,color:'var(--text-tertiary)',padding:'8px 12px',background:'var(--bg-elevated)',borderRadius:6,fontFamily:T.font.sans}}>
      ...
      <div style={card}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,paddingBottom:10,borderBottom:'1px solid var(--border-subtle)'}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <div style={{width:6,height:6,borderRadius:'50%',background:'var(--accent)',flexShrink:0}}/>
            <span style={{fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'0.06em',color:'var(--text-secondary)',fontFamily:T.font.sans}}>Virtual Net Metering — E.NA Ήρων</span>
          </div>
        ...
        {!vnmEnabled?(
          ...
          <>
            ...
            <div style={{background:'rgba(52,168,83,0.06)',border:'1px solid rgba(52,168,83,0.2)',borderRadius:8,padding:'12px 14px',fontSize:11,color:'var(--text-secondary)',lineHeight:1.6,fontFamily:T.font.sans}}>
      ...
      <div style={card}>
        ...
        {lastBillAmount&&lastBillKwh&&(
          <div style={{background:'var(--bg-elevated)',borderRadius:8,padding:'12px 14px',fontSize:11,color:'var(--text-secondary)',fontFamily:T.font.sans}}>
