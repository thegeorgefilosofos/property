        ...
        {docs.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => toggle(i)} aria-pressed={on} style={{
              ...
              padding: compact ? '8px 11px' : '11px 13px', borderRadius: 10, transition: 'background 0.15s, border-color 0.15s',
              ...
              <span style={{
                ...
                {on && <svg width={compact ? 11 : 12} height={compact ? 11 : 12} viewBox="0 0 24 24" fill="none" stroke="var(--accent-text)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>}
              </span>
              <span style={{ flex: 1, minWidth: 0 }}>
                <span style={{ fontSize: compact ? 12.5 : 13, fontWeight: 500, fontFamily: font, color: on ? 'var(--text-tertiary)' : 'var(--text-primary)', textDecoration: on ? 'line-through' : 'none' }}>{d.name}</span>
                {d.where && <span style={{ display: 'block', fontSize: compact ? 10.5 : 11, color: 'var(--text-tertiary)', fontFamily: font, marginTop: 2 }}>Από: {d.where}</span>}
              </span>
            </button>
          )
        })}
