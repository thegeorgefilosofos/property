    ...
    ? c.categories.map(([name, amt]) => `
      <div style="margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;font-size:12px;color:#3c4043;margin-bottom:4px">
          <span>${esc(name)}</span><span style="font-family:'Roboto Mono',monospace;font-weight:600">${eur(amt)}</span>
        </div>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Google Sans',system-ui,sans-serif;color:#202124;background:#fff;padding:40px;max-width:820px;margin:0 auto}
...
<body>
  <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #1a73e8;padding-bottom:18px;margin-bottom:8px">
    ...
    <div style="text-align:right"><div class="muted" style="font-size:11px">Ημερομηνία</div><div style="font-size:13px;font-weight:600">${today}</div></div>
  </div>
  ...
  <div class="sec"><span class="dot"></span> Ετήσιος Απολογισμός ${c.year}</div>
  <table>
