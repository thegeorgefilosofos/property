        ...
        <div className="sidebar-nav" style={{flex:1}}>
          {NAV_GROUPS.map((group,gi) => {
            ...
            <div className="sidebar-section" key={gi}>
              ...
              {open && group.ids.filter(id => isTabRelevant(effProfileType, id)).map(id => { const badge=getBadge(id); const locked=!isTabAllowed(ent, id); return (
        ...
        <header className="app-topbar">
          ...
          {selected ? (
            <>
              ...
              <button onClick={()=>{ if(typeof window!=='undefined') window.dispatchEvent(new Event('po:open-assistant')); }} title="Βοηθός" aria-label="Άνοιγμα βοηθού" style={{display:'flex',alignItems:'center',justifyContent:'center',width:36,height:36,borderRadius:'50%',border:'1px solid var(--border-default)',background:'transparent',color:'var(--text-secondary)',cursor:'pointer',marginRight:6}} onMouseEnter={e=>{e.currentTarget.style.background='var(--bg-hover)';e.currentTarget.style.color='var(--text-primary)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-secondary)'}}>
