        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          {needsConsent ? (
            ...
            <>
              ...
              {/* ── Η ΕΠΙΛΟΓΗ ΤΟΥ, ΓΡΑΜΜΕΝΗ ΣΑΝ ΑΠΟΔΕΙΞΗ ───────────────────────────
                  ΔΥΟ ΓΡΑΦΕΣ ΑΠΟΡΡΙΦΘΗΚΑΝ ΠΡΙΝ ΑΠΟ ΑΥΤΗ, ΚΑΙ ΑΞΙΖΕΙ ΝΑ ΞΕΡΕΙ
                  ΚΑΝΕΙΣ ΓΙΑΤΙ. Η πρώτη ήταν παράγραφος τριών προτάσεων γύρω από
                  μια κουκκίδα, με το ποσό θαμμένο στη μέση της πρώτης. Η
                  δεύτερη έσωσε τη στοίχιση αλλά έγραφε «Δοκιμή 30 ημερών, και
                  αλλάζεις πακέτο όποτε θέλεις» — σωστό, σύντομο, και εντελώς
                  άψυχο: δύο διαβεβαιώσεις κολλημένες με κόμμα, καμία τους στη
                  στιγμή που βρίσκεται ο αναγνώστης.

                  Η ΣΤΙΓΜΗ ΕΙΝΑΙ ΣΥΓΚΕΚΡΙΜΕΝΗ. Μόλις πάτησε ένα πακέτο σε
                  τιμοκατάλογο και ετοιμάζεται να γράψει email και κωδικό. Ξέρει
                  τι διάλεξε· δεν χρειάζεται να του το πούμε με ρήμα. Αυτό που
                  τον κρατά ένα δευτερόλεπτο πριν πληκτρολογήσει είναι μία μόνο
                  ερώτηση: πότε φεύγουν χρήματα.

                  Τρία επίπεδα, καθένα με μία δουλειά. Η ετικέτα λέει ΤΙ είναι
                  αυτό το κουτί. Η σειρά από κάτω είναι απόδειξη — όνομα
                  αριστερά, ποσό δεξιά, ίδια γραμμή βάσης, ισοπλατή ψηφία, πέρα
                  ως πέρα. Και η τελευταία γραμμή απαντά στην ερώτηση, με το
                  λεξιλόγιο που χρησιμοποιούν ήδη οι Οροι («δεν κινείται κανένα
                  ποσό»), αντί για δύο γενικές διαβεβαιώσεις.
                  ...
                  η χρέωση είναι ρυθμισμένη. Το «μέχρι να τελειώσει» είναι
                  αληθινό και στις δύο περιπτώσεις. */}
              {chosenPlan && (
                <div style={{ padding: '14px 16px 15px', marginBottom: 24, borderRadius: 10, background: 'var(--accent-dim)', border: '1px solid var(--accent-border)' }}>
                  <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 9 }}>
                    Το πακέτο σου
                  </div>
                  ...
                  <p style={{ margin: '9px 0 0', fontSize: 12.5, lineHeight: 1.55, color: 'var(--text-tertiary)' }}>
                    Οι πρώτες {TRIAL_DAYS} ημέρες είναι δοκιμή· κανένα ποσό δεν κινείται μέχρι να τελειώσει.
                  </p>
                </div>
              )}
