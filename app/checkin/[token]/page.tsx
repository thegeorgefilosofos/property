    ...
    <div style={{ background: 'var(--bg-base)', minHeight: '100vh', color: 'var(--text-primary)', fontFamily: "'Inter',sans-serif", paddingBottom: 40 }}>
      <header style={{ borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-surface)', marginBottom: 24 }}>
        <div style={{ ...wrap, height: 60, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 700 }}>Στοιχεία διαμονής</span>
        </div>
      </header>
      ...
      <div style={wrap}>
        ...
        {state === 'ok' && ctx && (
          <>
            <div style={card}>
              <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.02em', marginBottom: 2 }}>{ctx.property.name}</div>
            ...
            {sent ? (
              <div style={{ ...card, textAlign: 'center' }}>
                <div style={{ background: 'var(--positive-soft)', border: '1px solid var(--positive-border)', borderRadius: 10, padding: '16px', color: 'var(--positive)', fontSize: 15, fontWeight: 700 }}>✓ Ευχαριστούμε! Τα στοιχεία στάλθηκαν στον οικοδεσπότη.</div>
                <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 12, lineHeight: 1.6 }}>Καλό ταξίδι και καλή διαμονή!</div>
              </div>
            ) : (
              <div style={card}>
                <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Τα στοιχεία σου</div>
