...
| **Google** (Google LLC) | Three distinct live paths: Google OAuth sign-in (`app/login/page.tsx:56`, `app/signup/page.tsx:55`); embedded Maps iframe in the contact dossier (`proxy.ts:21`); Google Fonts requested by the browser when generating printable statements and reports (`statement.ts:99`, `accountingReport.ts:70`, `reportPdf.ts:57`, `TabChecklist.tsx:607`) | OAuth: email + Google account identity. Maps/Fonts: **user IP address and referrer** — this happens on every printable report, without the user choosing it | US (SCCs required) | ☐ execute (Google Cloud/Workspace DPA) |
