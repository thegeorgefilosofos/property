...
export const T = {
  // ── Η ΚΛΙΜΑΚΑ ΤΩΝ ΑΚΤΙΝΩΝ, ΓΡΑΜΜΕΝΗ ΟΠΩΣ ΠΡΑΓΜΑΤΙ ΕΙΝΑΙ ──────────────────
  //
  // ΤΙ ΕΛΕΓΕ ΠΡΙΝ. Εξι ονόματα, όλα ρόλοι: card 14, inner 10, btn 10, modal 18,
  // badge 100, pill 100. Και δίπλα τους, γραμμένα ωμά μέσα στις οθόνες, άλλες
  // τέσσερις τιμές που δεν υπήρχαν πουθενά στην κλίμακα — μετρημένες στα
  // `app/` και `components/` στις 24/08/2026:
  //
  //     10 → 157 φορές      8 → 102      6 → 74      3 → 65      12 → 57
  //
  // Δηλαδή η κλίμακα ΔΕΝ περιέγραφε το προϊόν. Το 8 και το 6 και το 3 και το
  // 12 δεν ήταν παρεκκλίσεις· ήταν το μεγαλύτερο μέρος του προϊόντος, και η
  // κλίμακα τα αγνοούσε. Οποιος άνοιγε αυτό το αρχείο για να δει «τι ακτίνες
  // έχουμε» έπαιρνε λάθος απάντηση, και έγραφε την επόμενη ακτίνα στην τύχη.
  //
  // ΓΙΑΤΙ ΔΥΟ ΕΙΔΗ ΟΝΟΜΑΤΩΝ ΚΑΙ ΟΧΙ ΕΝΑ. Τα `xs`…`lg` είναι η ΚΛΙΜΑΚΑ: οι
  // τιμές που όντως χρησιμοποιεί το προϊόν, χωρίς να ισχυρίζονται ρόλο. Τα
  // `card`/`inner`/`btn`/`modal`/`pill` είναι ΡΟΛΟΙ πάνω στην ίδια κλίμακα:
  // λένε τι είναι το στοιχείο, όχι πόσο στρογγυλεύει. Οταν ξέρεις τον ρόλο,
  // γράφεις τον ρόλο — αλλάζει μία γραμμή εδώ και αλλάζουν όλες οι κάρτες.
  // Οταν ΔΕΝ υπάρχει ρόλος (ένα δείγμα χρώματος 8×8, η κορυφή μιας στήλης
  // διαγράμματος), γράφεις το μέγεθος. Αυτό που δεν επιτρέπεται πλέον είναι
  // το τρίτο: αριθμός εκτός της κλίμακας (βλ. scripts/guard-radius-scale.mjs).
  //
  // ΤΟ ΜΗΔΕΝ ΕΙΝΑΙ ΚΙ ΑΥΤΟ ΑΠΟΦΑΣΗ, και ανήκει στην κλίμακα: μια πλευρά που
  // συνεχίζει στο διπλανό κελί ΔΕΝ στρογγυλεύει, και το γράφουμε ρητά.
  radius: {
    // κλίμακα
    none: 0, xs: 3, sm: 6, md: 8, lg: 12,
    // ρόλοι
    card: 14, inner: 10, badge: 100, btn: 10, pill: 100, modal: 18,
  },
...
export const fixedCols = (n: number, gap = 12, align: 'end' | 'start' | 'stretch' = 'end', cls = '') => ({
  ...
  style: {
    '--fc': n, '--fc-md': Math.min(n, 3), '--fc-sm': Math.min(n, 2),
    '--fc-gap': `${gap}px`, alignItems: align,
  } as Record<string, string | number>,
});

export const fieldRow = (min = 170, gap = 14) => ({
  display: 'grid' as const,
  gridTemplateColumns: `repeat(auto-fit, minmax(min(100%, ${min}px), 1fr))`,
  gap,
  // Τα πεδία έχουν ετικέτα από πάνω· χωρίς αυτό, ένα πεδίο με ετικέτα δύο
  // γραμμών σπρώχνει το κουτί του πιο κάτω από τα διπλανά του.
